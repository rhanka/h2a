...
- [~] V2: key management UX (user steer 2026-05-25) — complements signed-bearer transport. Slices: [ ] keyring store `registry/keys.jsonl` + multi-key verification (try ALL of an instance's registered keys, not just `publicKeys[0]`) + `h2a keys add/list` ; [ ] key revocation `h2a keys revoke` (rotate-out) ; [ ] rotation ergonomics/keyring polish. Enables key rotation: add new key (both verify during overlap) → revoke old.
