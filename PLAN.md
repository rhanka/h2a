...
- [~] V2: transport auth — **mechanism DECIDED: signed-bearer end-to-end** (user steer 2026-05-25; transport moves bytes, auth = ed25519 envelope signatures + nonce/timestamp anti-replay; no PKI/mTLS, survives relays/brokers). Slices: [x] signed envelopes in core (DEC-073) ; [ ] anti-replay (freshness window + nonce/id dedup, pure check + runtime guard) ; [ ] `@sentropic/h2a-remote` transport moving signed envelopes over HTTP/relay with verify-on-receipt. mTLS only as an optional deployment-layer add-on, not built now.
