...
> Last update: 2026-05-25 (DEC-076 — Transport auth slice 3b-i: HTTP `createRemoteServer`/`sendRemoteEnvelope` over the DEC-075 pipeline, end-to-end tested. V2 line 0.2.x. Next: slice 3b-ii = CLI verbs `h2a remote serve`/`send` wiring to store+registry)
...
- [~] V2: transport auth — **mechanism DECIDED: signed-bearer end-to-end** (user steer 2026-05-25; transport moves bytes, auth = ed25519 envelope signatures + nonce/timestamp anti-replay; no PKI/mTLS, survives relays/brokers). Slices: [x] signed envelopes in core (DEC-073) ; [x] anti-replay — `checkEnvelopeFreshness` + `createReplayGuard` (DEC-074) ; [~] remote transport (in `runtime/remote/`, **not** a 3rd package): [x] 3a receive-side trust boundary `acceptRemoteEnvelope` (DEC-075) ; [x] 3b-i HTTP `createRemoteServer` + `sendRemoteEnvelope` + status map, end-to-end tested with an ephemeral server (DEC-076) ; [ ] 3b-ii CLI verbs `h2a remote serve` (long-running) / `h2a remote send` wiring to store+registry. mTLS only as an optional deployment-layer add-on, not built now.
