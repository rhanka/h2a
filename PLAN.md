...
> Last update: 2026-05-26 (DEC-083 — **Drumbeat** (timed relances/escalation for crisis mgmt) specified & planned in `docs/drumbeat.md`, PRIORITY workstream; flagged gap: agy not an h2a host. Also: evaluations/BACKLOG.md (triple-review program + ISO 9001/27001/SOC2 + smart-contract evals to author); DEC-082 `/h2a model` skill (0.3.1); DEC-081 SysML interop spec.)
...
- [ ] V2: **Drumbeat — timed relances & escalation** (DEC-083, spec in `docs/drumbeat.md`) — **PRIORITY, specified & planned, not built**. Time-driven follow-up/escalation (advise→decide→alert on deadline) for crisis management. Runtime layer (pure `computeDrumbeatState` + `scanDrumbeat` + `h2a drumbeat`/MCP/`/h2a drumbeat`) + per-host scheduling layer. Slices D1-D4 (+ D0 optional `agy` host). Open: agy is an installed CLI but NOT an h2a host (hosts = claude/codex/gemini) — add `agy` host?
