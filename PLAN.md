...
- [~] V2: **Drumbeat — external anti-stall relance loop** (DEC-084, supersedes DEC-083; spec in `docs/drumbeat.md`) — **D1–D6 SHIPPED; only D7 left**. Core motivation: CLI agents stall (stop without finishing / out of tokens) and codex/agy/gemini have no `/loop` (Claude's self-wake can't revive a dead agent). External daemon `h2a drumbeat watch` (dumb anchor) reads presence `workStatus` + `launchContext` (captured at launch), relances via adapters (local-tmux priority → remote → headless), reflexive watchdog SUBAGENT decides, escalates to PRINCIPAL after N fails. Slices: [x] **D1 done (DEC-085, 0.4.0)** — presence `workStatus`+`launchContext` + pure `inferStall` ; [x] **D2 done (DEC-086, 0.5.0)** — durable registry + `scanDrumbeat` + `runDrumbeatWatch` + `H2ARelauncher` interface + `h2a drumbeat record/scan/clear/watch` ; [x] **D3 done (DEC-091)** — local-tmux + headless relaunchers (consume `launchContext`) ; [x] **D4 done (DEC-117, 0.21.0)** — remote relance relay: core `drumbeat.resume` + `remoteRelauncher` (signs/posts) + `relanceFromInbox` + `h2a drumbeat relance-inbox` + `watch --relauncher remote`/`auto=local-tmux→remote→headless` (delivered by `codex:a2a-cli`, Opus-reviewed) ; [x] **D5 done (DEC-111)** — reflexive watchdog subagent decider ; [x] **D6 done (DEC-104/113)** — per-CLI stop-hook plugins incl. agy ; [ ] D7 escalate-to-PRINCIPAL ; L2 deadlines later. agy = host (EVO-0, supports MCP).
- [ ] V2: SysML v2 interop (DEC-081, spec in `docs/sysml-interop.md`) — **specified & planned, not built**. h2a governs SysML v2 model deliverables by referencing immutable commits in signed envelopes (reuses signed-bearer transport). Slices: [ ] S1 pure `H2ASysmlRef` (core) ; [ ] S2 `runtime/sysml/` adapter (resolve/hash via API & Services) ; [ ] S3 `verifyEnvelopeSysmlRef` + `h2a sysml verify` ; [ ] S4 disclosure→view mapping. 4 open questions to resolve before S1.
