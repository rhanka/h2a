...
|---|---|---|---|---|---|
| B1 | **ISO 9001** (QMS) | cross-cutting | quality-management processes, internal audits, nonconformities, continual improvement → `ENGAGEMENT`/`POLICY`/`CONTROL`/`ENFORCEMENT_PLAN` + recurring obligations; the audit trail is the h2a journal | `evaluations/iso-9001.md` | ☐☐☐ |
| B2 | **ISO 27001** (ISMS) | cross-cutting | Annex A controls, risk treatment, Statement of Applicability → `POLICY` (imposed/contractual) + `CONTROL` + disclosure profile + evidence packages; signed engagements as control attestations | `evaluations/iso-27001.md` | ☐☐☐ |
| B3 | **SOC 2** (Trust Services Criteria) | cross-cutting | security/availability/confidentiality/processing-integrity/privacy → `CONTROL` + `attestation`/`evidence-package` disclosure + recurring-obligation cadence; the report period maps to the journal window | `evaluations/soc2.md` | ☐☐☐ |
