...
**C1 draft outcome** (2026-05-27): a **ranked P3 shortlist** — canonical in [`nhi-landscape.md` §6](./nhi-landscape.md#6-candidate-interop-targets-for-h2a-p3-reasoned-shortlist) — (1) SPIFFE/SPIRE (standards-first), (2) an NHI-governance platform as evidence-feed sink (vendor-bet caveat: Astrix mid-acquisition; **inbound posture-ingestion APIs unverified → a P3 prerequisite**), (3) secrets manager as rotation actuator; IETF WIMSE tracked as drafts-only (§7). **Since then**: shortlist #1 designed in the [P3 interop spec](../docs/superpowers/specs/2026-05-28-nhi-p3-interop-design.md) and shipped as `h2a nhi export` (DEC-094).
