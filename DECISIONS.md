...
## DEC-093 — Drumbeat D6 (slice a): per-host stop-hook plugin glue (`h2a host plugin`)
**Date**: 2026-05-28. **Refers**: DEC-054, DEC-084, DEC-085, DEC-091. **Line**: V2 (0.12.x).

**Context**: D2 relances a *recorded* stop and D3's local-tmux relauncher needs the pane captured at launch — but nothing yet records the stop with that context. The capture must happen at stop time inside the host's own terminal (where `$TMUX_PANE` + the session name exist). h2a cannot write into each host's runtime, so — exactly like `renderMcpConfig` — it renders the glue and the host runtime places it. Grounded in `docs/plugin-capability-matrix.md` (primary-source probe of all four CLIs), **at parity, no host privileged**.

**Decision**: `renderStopHook(host, {instance, root?, status?})` + `h2a host plugin` (resource).

- Per-host targets (`H2A_HOST_PLUGIN_TARGETS`): claude (`claude -c`, settings Stop hook, push), codex (`codex resume`, app-server, push), gemini (`gemini -r`, `gemini hooks`, push), **agy** (`agy -c`, plugin import, **poll-only** — no daemon, the matrix's one push gap → EVO-0 first-class here with a polling fallback).
- The rendered `record` command is `h2a drumbeat record --instance … --status paused --command/--resume-command <host resume> --tmux-session "$(tmux display-message -p '#{session_name}')" --tmux-pane "$TMUX_PANE" [--root]`, so the stop captures the launch context D3 sends back into the pane. Poll-only hosts also get a `poll` command (`h2a drumbeat scan`).

**Why**: (a) capturing at stop inside the host terminal is the only place `$TMUX_PANE`/session-name exist, and it is what D3 needs — rendering the exact command removes the guesswork; (b) following the `renderMcpConfig` pattern keeps h2a out of each host's runtime while still being prescriptive; (c) treating all four hosts symmetrically (agy included, with an honest poll fallback rather than a fake daemon) corrects the temptation to privilege the best-documented host; (d) the host's own resume command becomes the relauncher payload, tying D6 to D3.

**Consequence**: (a) `@sentropic/h2a-cli` exports `renderStopHook` + `H2A_HOST_PLUGIN_TARGETS`/`H2A_HOST_PLUGIN_HOSTS`; `h2a host plugin` is live and contract-tested (push hosts vs agy poll, unknown host → exit 1); (b) **additive surface → minor bump `0.12.0`** (shared with DEC-094); (c) the actual registration of the rendered hook into each host's config (settings.json / `gemini hooks` / codex app-server / agy plugin import) is the remaining D6 install step; D7 then consumes the recorded stops for escalation.

## DEC-094 — NHI P3 (slice a): SPIFFE-trust-bundle export (`h2a nhi export`)
**Date**: 2026-05-28. **Refers**: DEC-078, DEC-087, DEC-088, DEC-090. **Line**: V2 (0.12.x).

**Context**: the veille (`evaluations/nhi-landscape.md`) ranked **SPIFFE/SPIRE** the #1 P3 interop target — h2a's keyring (active public keys, rotated add→overlap→revoke) maps onto a SPIFFE trust bundle, and the signed attestation envelope onto a verifiable credential. This ships the first interop primitive.

**Decision**: a pure core transform + a read-only verb/tool.

- **Core** (`@sentropic/h2a`, `nhi-export.ts`): `nhiSpiffeId(trustDomain, instance)` → `spiffe://<td>/<path>` (validates/lowercases the trust domain, encodes h2a id chars illegal in a SPIFFE path: `:`→`.`, `~`→`--`); `nhiTrustBundle({instance, trustDomain, activeKeys, …})` → a SPIFFE-trust-bundle / JWKS-**shaped** object (`keys`, optional `spiffe_sequence`/`spiffe_refresh_hint`, `spiffe_id`, `trust_domain`); each key entry is JWK-shaped (`kid` = `nhiKeyFingerprint`, `kty:"OKP"`) but the key material/use are **h2a-namespaced** (`h2a_public_key_pem`, `h2a_use:"h2a-envelope-signing"`).
- **Honesty boundary** (per the spec): h2a holds PEM ed25519 keys, not JWK/SVID material, so core exports **trust-anchor** material in a bundle shape rather than masquerading as an `x509-svid`/`jwt-svid`. PEM→JWK(OKP) conversion, SVID minting and the live HTTPS bundle endpoint are assigned to an **external connector in `../sentropic/`**, keeping core dependency-free.
- **CLI**: `h2a nhi export --instance <id> --trust-domain <td>` (resource; exit 1 on invalid trust domain). **MCP**: `h2a_nhi_export`.
...
## DEC-103 — Drumbeat D6 (slice b cont.): `host plugin --write` extended to gemini
**Date**: 2026-05-28. **Refers**: DEC-093, DEC-102. **Line**: V2 (0.14.x).

**Context**: DEC-102 shipped `host plugin --write` for claude only. Probing the other three for tonight's multi-host run: **gemini** exposes `gemini hooks migrate --from-claude` and reads hooks from `~/.gemini/settings.json` → it accepts the **Claude-format `hooks.Stop`** structure. **codex** has a hook mechanism (`--dangerously-bypass-hook-trust`) but it is plugin/hook-trust-based (`~/.codex/.../hooks`, plugin.json), not a single settings-file merge. **agy** has no daemon (poll-only).

**Decision**: extend the `--write` install path to **gemini** (same idempotent `hooks.Stop` merge as claude, written to `~/.gemini/settings.json`); keep **codex** and **agy** as render + refuse-`--write` (codex registers via its plugin/hook-trust mechanism; agy polls `h2a drumbeat scan`/`blockage list`). The rendered `record` already carries each host's resume command (`claude -c`/`gemini -r`/`codex resume`/`agy -c`).
...
## DEC-108 — EVO-8: in-place re-exec on auto-upgrade + opt-in boot check
**Date**: 2026-05-28. **Refers**: DEC-105, DEC-107. **Line**: V2 (0.17.x).

**Context**: DEC-107's `--auto-upgrade` only applied "next launch". The user asked for an immediate restart. Node ≥ 23.10 exposes `process.execve` (verified present on the dev box, Node v24.15.0), which **replaces the process image while preserving PID + stdio fds** — so a host-spawned `mcp-serve` can re-exec into the new binary and the host's MCP connection survives (a plain spawn+exit would not — the host sees its child die).

**Decision**:
- **`reexecSelf()`** (`runtime/upgrade/`): after a successful boot-time install, re-exec `process.execPath` with the same argv, setting `H2A_UPGRADE_REEXECED=1` so the new image does not re-upgrade (loop guard). **CRITICAL safety**: a failing `process.execve` aborts *natively* (not a catchable throw), so `reexecSelf` guards with `existsSync(execPath)` before calling it and only ever targets `process.execPath` (always valid). `--no-restart` keeps the next-launch behavior; non-POSIX/old Node (no `execve`) falls back automatically.
- **Boot check is now OPT-IN** (reversing DEC-107's default-on): `mcp-serve` does the registry check only with `--auto-upgrade` (upgrade + re-exec) or `--upgrade-check` (notice only). **A default boot does no network** — the prior default-on check added an `npm view` to every server start, which (a) is an unwanted phone-home and (b) hung the spawn-based tests. `npm view`/`npm i` spawnSync calls are bounded (4s / 120s timeouts).

**Why**: (a) `execve` is the only mechanism that applies the upgrade in the *same* session without the host seeing a disconnect — honest immediacy, with a clean fallback; (b) the existsSync guard is load-bearing because execve failure is a hard native abort; (c) opt-in boot-check removes network from the default startup path (correctness + the test hang) while the user's `--auto-upgrade` config still gets the behavior.

**Consequence**: (a) `reexecSelf`/`canReexec` + the `H2A_REEXEC_GUARD_ENV` exported; `mcp-serve` gains `--upgrade-check`/`--no-restart`, drops `--no-upgrade-check` (default is now off); (b) 13 upgrade tests (incl. re-exec records/guard/missing-target/throws) + full suite 564 green; (c) **additive → 0.17.x**; (d) the user's host configs already pass `--auto-upgrade` (no `--no-restart`) → they get immediate in-place restart on the next published version.

## DEC-109 — EVO-7 slice 2: org-manifest parser + `h2a org` / `h2a coach` (read-only)
**Date**: 2026-05-29. **Refers**: DEC-106. **Line**: V2 (0.18.x).

**Context**: DEC-106 shipped the pure org model (`H2AOrgManifest` + `validateOrgManifest` + `orgAssignmentEnvelope`) but left the *committed* `org.h2a.yaml` form and any CLI surface to later slices. This slice makes the committed manifest readable + checkable + proposable, still without any live mutation.

**Decision**:
- **`parseOrgManifest(text)`** (`@sentropic/h2a`, new `org-parse.ts`): a **pure, zero-dependency** reader for a small **block-YAML subset** (block maps, block sequences `- `, flow sequences `[a, b]`, single/double-quoted scalars, `#` comments, blank lines) — or **JSON** when the document starts with `{`/`[` (so `org.h2a.json` works). h2a has **no runtime deps**, so pulling in a YAML library was rejected in favour of a focused subset parser that rejects anything outside the subset (e.g. tab indentation) with a line-anchored error. Total: never throws → `{ manifest?, errors[] }`. Shape-coercion checks manifest *types* only; the h2a invariants stay in `validateOrgManifest`.
- **`h2a org validate [--file]`** (default `org.h2a.yaml` in cwd) → parse + invariants → `{ ok, errors }`; exit 1 on parse/invalid, exit 3 on unreadable file. **`h2a org show [--file]`** → normalized manifest + its `validation` block.
- **`h2a org diff [--file] [--root]`** → reconcile the declared manifest against the live registry (`listInstances`): `{ scope, matched, missing, undeclared, roleMismatch, scopeGaps, inSync }` via the pure `diffOrgManifest(manifest, registered)` in core. Read-only — reports what provisioning *would* change. All three `org` verbs are read-only.
- **`h2a coach propose --as <coach> [--file] [--role CONDUCTOR] [--scope]`** → emits the **unsigned** `org-proposal` envelope (via `orgAssignmentEnvelope`) for a *valid* manifest; refuses an invalid one (exit 1). The coach **proposes, does not impose**.
- **`h2a coach ratify --as <principal> --private-key <pem> [--file] [--role PRINCIPAL] [--scope]`** → the PRINCIPAL ratifies: emits the **ed25519-signed** `org-ratified` envelope (`signEnvelope`, mirrors `nhi attest`), verifiable with `verifyEnvelopeSignature`. Refuses an invalid manifest (exit 1). This completes the propose→ratify lifecycle the user specified (coach proposes → PRINCIPAL ratifies). Neither verb mutates the store — output only.

**Why**: (a) the zero-dep subset parser keeps the 2-package / no-runtime-dep posture intact while honouring the user's `org.h2a.yaml` naming; (b) org tooling lives as **CLI-only** verbs (like `deploy`/`host`/`keys`) — the MCP tools are for live runtime coordination, which this read-only slice deliberately isn't; (c) emitting an *unsigned* proposal keeps the propose→counter→ratify lifecycle (and the private key) out of this slice.

**Consequence**: (a) `@sentropic/h2a` exports `parseOrgManifest` / `diffOrgManifest` / `effectiveOrgInstances` / `H2A_ORG_MANIFEST_FILENAME` (+ `H2AOrgParseResult`, `H2AOrgDiff`, `H2AOrgMembershipGrant`, `H2AOrgRegisteredInstance`, `YamlValue`); `@sentropic/h2a-cli` adds the `org validate`/`org show`/`org diff`/`org provision`/`coach propose`/`coach ratify` verbs (+ `cmdOrg`/`cmdCoach`) and the store's `grantOrgMembership`/`listOrgMembership` (new `registry/org-membership.jsonl`); (b) 13 parser + 9 diff/effective + ~14 CLI tests + contract happy-paths, full suite 604 green; (c) **additive → 0.18.x**; (d) the full propose→ratify→provision lifecycle ships — EVO-7 is functionally complete.

**Provisioning fork resolved (PRINCIPAL's call) — shipped**: `org provision` applies a ratified org **reconcile keyed-only**: for each declared instance already registered it appends role/scope membership **grants**; a declared-but-unregistered instance is reported under `pending` ("needs key + register"), never auto-stubbed (keeps the NHI4/NHI5 posture clean; reversible — auto-enrolment can be added later). The mechanism follows the **DEC-078 keyring pattern**: grants live in a *separate append-only event log* (`registry/org-membership.jsonl`), never a registration rewrite; `effectiveOrgInstances(registrations, grants)` unions them so `org diff` reads a provisioned org back in-sync (idempotent — re-provisioning grants nothing new). **Remaining (later, optional)**: surface the effective view to `discover`/scope-gating at runtime; the propose→counter→ratify MCP flow; grant revocation for true (non-additive) reconciliation.

## DEC-110 — EVO-7: runtime scope-gating from the effective view + proposal delivery (no "optional remainder")
**Date**: 2026-05-29. **Refers**: DEC-109, DEC-078. **Line**: V2 (0.18.x).

**Context**: DEC-109 parked two EVO-7 pieces as "optional/later". The PRINCIPAL's call: *optional = never done* — so they are finished here, making provisioning actually affect runtime instead of being cosmetic.

**Decision**:
- **Effective-view gating**: `discover` (CLI) and `h2a_discover_instances` (MCP) now filter on `effectiveOrgInstances(listInstances(), listOrgMembership())` — a `provision`-granted role/scope gates discovery at runtime. Output stays the raw registration; grants only **widen** the match set, never narrow it (non-breaking: registration scopes ⊆ effective).
- **`--deliver`** on `coach propose`/`coach ratify`: drops the (un)signed org envelope into each declared instance's inbox (`putInboxMessage`), closing the propose→counter→ratify loop over the existing mailbox primitives (agents read the proposal, counter via inbox, the PRINCIPAL ratifies). The envelope stays the stdout payload; delivery is reported on stderr (the `resource` contract is preserved).

**Why**: (a) without effective-view gating, provisioning grants changed nothing observable — now they change who-discovers-whom; (b) `--deliver` reuses the inbox rather than inventing an MCP **signing** surface (server-side key custody is a separate, project-wide concern — deliberately NOT introduced for org alone); (c) filter-widen-only keeps discover non-breaking.

**Consequence**: (a) discover + `h2a_discover_instances` consult grants; `coach propose`/`ratify` gain `--deliver`/`--root`; (b) full suite 606 green; (c) **additive → 0.18.x**; (d) **EVO-7 is fully complete — no optional remainder.** The only genuinely-separate future item is MCP tools that *sign* (org or otherwise), blocked on server-side key custody — a project-wide decision, not an EVO-7 gap.

## DEC-111 — Drumbeat D5: reflexive watchdog
**Date**: 2026-05-30. **Refers**: DEC-086 (drumbeat), DEC-091 (relaunchers), DEC-095 (escalation/D7), DEC-092 (blockage/EVO-3). **Line**: V2 (0.19.x).

**Context**: `drumbeat watch` was binary — relance every stalled agent until the cap, then escalate. D5 adds a **judgment step**: a reflexive watchdog decides, per stalled agent, between relance / finish / escalate / reroute. Brainstormed + ratified, then **hardened by an independent spec/plan review** that caught five high-impact flaws before any code.

**Decision** (design: `docs/superpowers/specs/2026-05-29-d5-reflexive-watchdog-design.md`; plan: `docs/superpowers/plans/2026-05-30-drumbeat-d5-reflexive-watchdog.md`):
- **Core** `parseReflexiveDecision` (pure, total; unknown/garbage → `relance`). **Pluggable `ReflexiveDecider`** (like relaunchers): `loggingDecider` (opt-out default = today's behaviour) + `subagentDecider` (headless host CLI; prompt passed as an **argv argument**, stdin closed, agent-controlled fields **delimited as untrusted data**; any failure → `relance`).
- **Opt-in** `--decider`; **cost guard** `--decider-after K` (default 1; validated `< --max-relances`) — relance cheaply first, judge only in `[K, max-relances)`; **advisory-first** — by default the verdict is logged but the safe default (`relance`) is applied; actions apply only with **`--decider-enforce`**.
- Dispatch: `relance` (counts to the cap); `finish` clears **only** when `workStatus === "done"` else `escalate` (`done` is the only safe-to-finish status — `paused` is the stop-hook's *unexpected-stop* default, so it must not auto-clear); `escalate` and `reroute` route through the escalation registry (reasons `watchdog-escalate` / `reroute-suggested`) and **mark the entry terminal** (`scanDrumbeat` skips terminal entries → no re-decide; not a destructive unlink that a later `recordStop` re-arms). **`reroute` = escalate-with-hint in v1** (a peer-blockage hand-off is a verified no-op until EVO-3 supports ownership transfer). Every decision is written to an **append-only `decisions.jsonl`** (`decided` vs `applied`).

**Why**: (a) advisory-first + opt-in + `relance` fallback make the watchdog never-worse-than-today and reversible; (b) the review-driven corrections avoid real bugs (stdin prompt = no-op adapter; `paused`-finish = silent abandonment; empty-scope reroute-blockage = reaches no one; unlink+re-record = slow loop; missing K-gate validation = silent dead-zone); (c) terminal-marking + the audit log keep the loop safe and observable.
...
**Decision**:
- **VALEUR** = ENGAGEMENT field + derived chain (Fork A). Two **additive optional** fields on `H2AEngagement`: `aval` (downstream engagement id) + `finaliteAmont` (upstream finalité served). They stay within the ENGAGEMENT profile and **pass the contractual collapse-guard** (`auditContractualArtifact` — no collision with CONTRACT/POLICY field sets); a test proves it. New pure `value-chain.ts`: `deriveValueChain(engagements, fromId, { disclosureMode? })` traverses `aval` as a linked list, guards cycles via a visited set, and — per review **F8** — marks a terminal node `boundaryOpaque: true` when its onward link is unresolved under an **opaque** disclosure mode (anything but `full-view`), rather than silently returning a partial chain as complete. Total, never throws. The chain is **derived, never stored**.
- **MUTUALISATION** = pure advisory, the **positive mirror of NHI9**. New pure `mutualisation.ts`: `deriveMutualisationOpportunities(instances)` over the `H2AOrgRegisteredInstance` shape (`instance` + `scopes`) surfaces one capitalisation candidate per scope held by ≥2 instances. Peer of `nhiInventory`: **surfaces candidates, obligates nothing**. Deterministic (ordered by scope, instances sorted), input-order independent, total.

**Why**: (a) one additive ENGAGEMENT field pair (no new artifact/role) is the lightest frozen-surface touch (Fork A rejected a dedicated artifact as heavier); (b) deriving the chain keeps VALEUR a *calculation*, consistent with the NHI posture pattern; (c) F8's `boundaryOpaque` keeps the derivation honest across opaque cross-org boundaries (no silent partial-as-complete); (d) MUTUALISATION as advisory respects "the engine surfaces + routes, never judges/obligates"; (e) collision-free identifiers — `aval`/`finaliteAmont`/`mutualisation`, deliberately **never** `interest`/`interests` (reserved for `H2ASessionInterests`).

**Consequence**: (a) `@sentropic/h2a` adds optional `aval`/`finaliteAmont` on `H2AEngagement`, exports `deriveValueChain` (+ `H2AValueChainNode`, `H2ADeriveValueChainOptions`) and `deriveMutualisationOpportunities` (+ `H2AMutualisationOpportunity`); (b) +21 tests (12 VALEUR, 9 MUTUALISATION), full suite green; (c) **additive → 0.19.x**; (d) **out of scope here**: CLI/MCP surfacing of either derivation, and the ATTENTION/INTÉRÊT/CONFIANCE slices (later, per the stabilized sequence). No version bump / no merge in this slice — integrated after review.

## DEC-113 — Drumbeat D6 (done): scaffold the full codex stop-hook plugin (`host plugin --scaffold`)
**Date**: 2026-05-30. **Refers**: DEC-093, DEC-102, DEC-103, DEC-104. **Line**: V2 (unreleased — pending review/integration).

**Context**: DEC-104 closed D6 across the supported surface but left one honest gap: codex's `--write` produced only a valid `hooks.json` + a `pluginHint`; a codex user still had to hand-create the `plugin.json` manifest and trust it, so codex was not installable one-shot like claude/gemini. The remaining D6 automation was to scaffold codex's plugin wrapper. Probed codex's **actual** plugin/trust model live (`codex plugin {marketplace add, list, add}`; `~/.codex/plugins/.../.codex-plugin/plugin.json`; `~/.codex/superpowers/{.claude-plugin,.cursor-plugin}/plugin.json` with `"hooks": "./hooks/hooks.json"`; trust recorded in `~/.codex/config.toml` as `[plugins."<name>@<mkt>"] enabled = true`).

**Findings (codex trust mechanism, live-verified)**: codex has **no drop-in plugin directory** and **no `--dangerously-bypass-hook-trust` flag** in the shipped CLI (the flag the old `pluginHint` referenced does not exist). Crucially, codex installs a plugin **only from a local *marketplace* directory, not a bare plugin dir** — `codex plugin marketplace add <bare-plugin-dir>` is rejected with *"marketplace root does not contain a supported manifest"*. The marketplace layout (mirrored from codex's own `openai-curated`) is: `<dir>/.agents/plugins/marketplace.json` (manifest: `{ name, interface?, plugins:[{ name, source:{source:"local",path:"./plugins/<name>"}, policy:{installation:"AVAILABLE",authentication:"ON_INSTALL"|"ON_USE"}, category }] }` — `authentication:"NONE"` is rejected) + `<dir>/plugins/<name>/.codex-plugin/plugin.json` (plugin manifest, `hooks:"./hooks/hooks.json"`) + `<dir>/plugins/<name>/hooks/hooks.json` (Claude-format Stop hook). The marketplace **name is taken from `marketplace.json.name`** (so it IS controllable — `marketplace add` registered the h2a scaffold as `h2a-local`). Loading is a two-step CLI flow: `codex plugin marketplace add <dir>` → `codex plugin add <name>@<marketplace>` (writes `[plugins."<name>@<mkt>"] enabled = true` in `~/.codex/config.toml`). Verified end-to-end: the h2a-scaffolded marketplace was accepted by `marketplace add` and `codex plugin list` then showed `h2a-drumbeat@h2a-local not installed`, ready for `plugin add`. h2a cannot safely pre-write the config trust line, so trust is **surfaced, not faked**.

**Decision**: add `h2a host plugin --host codex --scaffold <dir>` — writes codex's **full local marketplace**: `<dir>/.agents/plugins/marketplace.json`, `<dir>/plugins/h2a-drumbeat/.codex-plugin/plugin.json` (`hooks:"./hooks/hooks.json"`), and `<dir>/plugins/h2a-drumbeat/hooks/hooks.json` (the same idempotent Claude-format `hooks.Stop` merge as `--write`, via the extracted `mergeStopHooksFile`). The output envelope carries `ok/host/scaffolded/marketplace/manifest/hooks/mechanism/hook` plus a **`trust` array** with the exact, runnable two-command step (`codex plugin marketplace add <dir>` → `codex plugin add h2a-drumbeat@h2a-local`). `--scaffold` is **codex-only** (the manifests are codex-specific — claude/gemini merge a single settings.json via `--write`; agy polls), refused otherwise. The stale codex `pluginHint` on `--write` is refreshed to point at `--scaffold`. Pure helpers `codexPluginManifest`/`codexMarketplaceManifest`/`codexPluginTrustCommands` + `H2A_CODEX_PLUGIN_NAME`/`H2A_CODEX_MARKETPLACE_NAME`/`H2A_CODEX_PLUGIN_HOOKS_REL` in `hosts/plugin.ts`.

**Why**: (a) scaffolding the marketplace from the **live-verified** codex layout makes codex installable as close to one-shot as codex's trust model allows — at parity with claude/gemini; (b) emitting the trust commands (rather than guessing a config write) keeps the honest boundary — codex genuinely requires the user-run trust step, and it is in the action envelope, not a silent gap; (c) reusing `mergeStopHooksFile` keeps the hooks.json byte-identical to `--write codex` and idempotent on re-scaffold; (d) codex-only refusal avoids inventing a codex manifest for hosts that don't use one; (e) the bare-plugin-dir scaffold (initial implementation) was corrected to a marketplace after `marketplace add` rejected it — the decision is grounded in what codex actually loads, not the docs.

**Consequence**: (a) `host plugin` gains `--scaffold <dir>` (codex full marketplace + trust step); `mergeStopHooksFile` shared by `--write`/`--scaffold`; the codex manifest/marketplace/trust helpers added to `hosts/plugin.ts`; (b) **D6 is complete** — claude/gemini one-shot settings.json, codex one-command scaffold + the two-command trust step codex mandates (live-verified), agy poll; (c) +2 host-plugin tests (codex full-marketplace layout + trust array + idempotence; non-codex refusal), contract `optionalFlags` gains `scaffold`, full suite green; (d) **additive surface**; no version bump / no merge in this slice — integrated after review.

## DEC-114 — fix: `fetchLatest` 4s timeout silently broke every upgrade check
**Date**: 2026-05-30. **Refers**: DEC-107/DEC-108 (EVO-8 auto-upgrade). **Line**: V2 (0.20.1).

**Context**: dogfooding 0.20.0 revealed the global CLI stuck at **0.18.0** despite 0.19.0/0.20.0 being published. Diagnosis: `defaultUpgradeRuntime.fetchLatest` ran `spawnSync("npm", ["view", pkg, "version"], { timeout: 4000 })`, but `npm view` routinely takes **~5.5s** on this machine → the spawn **always timed out** → `fetchLatest` returned `undefined` → `checkUpgrade` reported `upgradeAvailable:false` ("registry unreachable"). The **same `fetchLatest`** backs both `h2a upgrade` and the `mcp-serve --auto-upgrade` boot check, so **the entire EVO-8 auto-upgrade promise was silently broken** for anyone whose `npm view` exceeds 4s (common — `npm view` is slow).
...
**Consequence**: (a) `@sentropic/h2a` exports the D4 resume body parser/constants; `@sentropic/h2a-cli` exports `remoteRelauncher`, `relanceFromInbox`, and the receive result types; (b) CLI adds `drumbeat relance-inbox` and `drumbeat watch --relauncher remote`; (c) D4 tests cover parser, remote relay success/failure, inbox consume idempotence, CLI consume, the signer-key validation, and an end-to-end POST through `remoteServerForStore` (signed delivers + consumes; unsigned/wrong-key is rejected at ingestion and never relances); (d) deferred options remain out of scope: direct remote-exec on `serve` needs its own security DEC, and broker-managed respawn belongs to the remote bridge track.

**Authorization model (authentication-only)** — adversarial-review hardening: a peer whose signature is verified against the registry at `acceptRemoteEnvelope` *before* delivery may request a relance of **any** non-terminal local drumbeat entry; there is no per-entry ownership or role gate. The blast radius is bounded by design — only already-known stopped/out-of-tokens entries, the **receiving** host's own captured `launchContext` (a remote requester cannot inject a command, only nudge a known agent), and the relance-count cap. As accountability hardening the consumer also asserts `body.requestedBy === envelope.actor.instance` (the cryptographically verified signer) and drops a `requester-mismatch` envelope, so a relance is never recorded under a forged requester. A finer ownership/role gate, if ever required, is a follow-on and not a security blocker (the transport never grants remote-execution authority).
