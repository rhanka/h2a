...
**Consequence**: (a) `@sentropic/h2a` exports `parseOrgManifest` / `diffOrgManifest` / `H2A_ORG_MANIFEST_FILENAME` (+ `H2AOrgParseResult`, `H2AOrgDiff`, `H2AOrgRegisteredInstance`, `YamlValue`); `@sentropic/h2a-cli` adds the `org validate`/`org show`/`org diff`/`coach propose`/`coach ratify` verbs (+ `cmdOrg`/`cmdCoach`) and contract entries; (b) 13 parser + 6 diff + ~13 CLI tests (incl. ratify-signature-verifies) + contract happy-paths, full suite 599 green; (c) **additive → 0.18.x**; (d) the propose→ratify governance lifecycle is now complete.

**Provisioning fork resolved (PRINCIPAL's call)**: the *next* slice — `org provision` (apply a ratified org to the live estate) — chooses **reconcile keyed-only**: it never fabricates unauthenticated identities; a declared-but-unregistered instance is reported as "needs key + register", not auto-stubbed (keeps the NHI4/NHI5 posture clean; reversible — auto-enrolment can be added later). The mechanism follows the **DEC-078 keyring pattern**: role/scope membership changes will be a *separate append-only event log* (`registry/org-membership.jsonl`) layered over registrations (never a registration rewrite), consulted by `listInstances`/`discover`/`diff`/scope-gating. That event-log design + the propose→counter→ratify MCP flow are the next EVO-7 slice.
