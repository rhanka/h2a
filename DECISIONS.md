...
- **`h2a org validate [--file]`** (default `org.h2a.yaml` in cwd) → parse + invariants → `{ ok, errors }`; exit 1 on parse/invalid, exit 3 on unreadable file. **`h2a org show [--file]`** → normalized manifest + its `validation` block. Both read-only.
...
**Consequence**: (a) `@sentropic/h2a` exports `parseOrgManifest` / `H2A_ORG_MANIFEST_FILENAME` (+ `H2AOrgParseResult`, `YamlValue`); `@sentropic/h2a-cli` adds the `org validate`/`org show`/`coach propose` verbs (+ `cmdOrg`/`cmdCoach`) and contract entries; (b) 13 parser tests + 9 CLI tests + contract happy-paths, full suite 588 green; (c) **additive → 0.18.x**; (d) next EVO-7 slice: provisioning the *ratified* manifest (register instances + scope membership + mandates) and the propose→counter→ratify MCP flow.
