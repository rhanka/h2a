...
**Findings (codex trust mechanism)**: codex has **no drop-in plugin directory** and **no `--dangerously-bypass-hook-trust` flag** in the shipped CLI (the flag the old `pluginHint` referenced does not exist). A plugin is a directory with a `.codex-plugin/plugin.json` manifest (codex-native; codex also reads `.claude-plugin`/`.cursor-plugin` manifests) whose `hooks` key points at a Claude-format `hooks.json`. Loading it is a two-step CLI flow: `codex plugin marketplace add <local-dir>` registers the dir as a local marketplace (the marketplace **name is derived by codex, not caller-named** — there is no `--name` flag), then `codex plugin add <plugin>@<marketplace>` installs + enables it (writing the `enabled = true` trust line). h2a cannot safely pre-write that marketplace snapshot/config, so trust is **surfaced, not faked**.

**Decision**: add `h2a host plugin --host codex --scaffold <dir>` — writes codex's **full plugin**: `<dir>/.codex-plugin/plugin.json` (manifest with `name`/`version`/`description` + `hooks: "./hooks/hooks.json"`) and `<dir>/hooks/hooks.json` (the same idempotent Claude-format `hooks.Stop` merge as `--write`, via the extracted `mergeStopHooksFile`). The output envelope carries `ok/host/scaffolded/manifest/hooks/mechanism/hook` plus a **`trust` array** with the exact, runnable commands (`codex plugin marketplace add <dir>` → `codex plugin marketplace list` → `codex plugin add <name>@<marketplace>`). `--scaffold` is **codex-only** (the manifest is codex-specific — claude/gemini merge a single settings.json via `--write`; agy polls), refused otherwise. The stale codex `pluginHint` on `--write` is refreshed to point at `--scaffold`. Pure helpers `codexPluginManifest`/`codexPluginTrustCommands` + `H2A_CODEX_PLUGIN_NAME` in `hosts/plugin.ts`.

**Why**: (a) scaffolding the manifest + hooks.json from primary-source-verified codex layout makes codex installable as close to one-shot as codex's trust model allows — at parity with claude/gemini; (b) emitting the trust commands (rather than guessing a config/marketplace-snapshot write) keeps the honest boundary — codex genuinely requires the user-run trust step, and it is in the action envelope, not a silent gap; (c) reusing `mergeStopHooksFile` keeps the hooks.json byte-identical to `--write codex` and idempotent on re-scaffold; (d) codex-only refusal avoids inventing a codex manifest for hosts that don't use one.

**Consequence**: (a) `host plugin` gains `--scaffold <dir>` (codex full plugin + trust step); `mergeStopHooksFile` shared by `--write`/`--scaffold`; `codexPluginManifest`/`codexPluginTrustCommands`/`H2A_CODEX_PLUGIN_NAME` added to `hosts/plugin.ts`; (b) **D6 is complete** — claude/gemini one-shot settings.json, codex one-command scaffold + the trust step codex mandates, agy poll; (c) +2 host-plugin tests (codex full-scaffold layout + trust array + idempotence; non-codex refusal), contract `optionalFlags` gains `scaffold`, full suite green; (d) **additive surface**; no version bump / no merge in this slice — integrated after review.
