...
- **`h2a coach propose --as <coach> [--file] [--role CONDUCTOR] [--scope]`** → emits the **unsigned** `org-proposal` envelope (via `orgAssignmentEnvelope`) for a *valid* manifest; refuses an invalid one (exit 1). The coach **proposes, does not impose** — signing, persistence, PRINCIPAL ratification and provisioning are later slices.
...
**Consequence**: (a) `@sentropic/h2a` exports `parseOrgManifest` / `diffOrgManifest` / `H2A_ORG_MANIFEST_FILENAME` (+ `H2AOrgParseResult`, `H2AOrgDiff`, `H2AOrgRegisteredInstance`, `YamlValue`); `@sentropic/h2a-cli` adds the `org validate`/`org show`/`org diff`/`coach propose` verbs (+ `cmdOrg`/`cmdCoach`) and contract entries; (b) 13 parser + 6 diff + ~10 CLI tests + contract happy-paths, full suite 596 green; (c) **additive → 0.18.x**; (d) next EVO-7 slice (the one genuine design fork): provisioning the *ratified* manifest — how a declared instance maps onto the keyed identity registry (auto-register vs reconcile-and-require-keys) — plus the propose→counter→ratify MCP flow.
