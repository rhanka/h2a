...
1. **Threat model**: is a hostile/buggy **same-user local process** in scope? If yes → proof-of-possession (F1) is mandatory + private key needs at-rest protection (file perms / OS keystore). If "single trusted user, same machine" is declared out of scope → F1 is defense-in-depth and the env anchor can ship with a documented caveat. **This one call sizes the whole slice.**
   - **RATIFIED 2026-05-31 (PRINCIPAL)**: *"single trusted user, same machine"* — a hostile/buggy local process under the same user is **OUT of scope**. PoP (the binding+PoP unit already built) stays as **defense-in-depth**, not a mandatory boot-time nonce challenge: the local boot path reclaims on `(host, providerSession, workspaceId)` + local-key possession, **no interactive nonce challenge**. **Caveat (PRINCIPAL): "always think remote too"** — the `remote` host crosses the trust boundary, so it does NOT get the relaxed local reclaim: a bridge session stays **always-mint** (ephemeral Pod, no persistent keystore — spec §Remote L185-191), and binding-key separation prevents a local process from reclaiming a remote identity. A genuine key-anchored remote NHI remains the separate paired slice in `rhanka/remote`.
