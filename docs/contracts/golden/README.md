...
## Fichiers
- **`mcp-tools.json`** — les 38 outils MCP `h2a_*` exposés sur le bus local (surface du bus, dont les mutateurs local-only `h2a_run` et `h2a_send`). Source : `h2a mcp-tools` (trié).
- **`cli-verbs.json`** — les 99 verbes dispatchables de la CLI. Source : `H2A_CLI_VERB_CONTRACTS` (`packages/h2a/src/cli-contract.ts`, trié). Évolution : `keys prove-control` (PR #30, 97 → 98), `explain` (98 → 99), `loop enable-auto-tick` (99 → 100), puis le retrait de `report-ai` et `report-ai install-track-config` (100 → 98), et `send` (98 → 99). Le chemin cité pointait sur `packages/h2a-cli/`, qui est un stub déprécié.
...
`H2A_CLI_VERB_CONTRACTS` et le tableau `expected` ont fusionné proprement à 99, puis
`loop enable-auto-tick` a porté le contrat à 100, tandis
