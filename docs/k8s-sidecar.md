...
> Scenario A of [DEC-056](../DECISIONS.md#dec-056--instruction-note-k8s-deployment--remote-interop), implemented by DEC-058.

This page describes how to embed `h2a mcp-serve` as a Pod sidecar so a CLI agent running inside a Kubernetes Pod (typically a [`@sentropic/remote`](https://github.com/rhanka/remote) session Pod) can join the h2a coordination bus.
...
## Identity bridge with `@sentropic/remote`

The sidecar exports three env vars used by the agent on `h2a_session_open`:

| Variable | Default | Role |
|---|---|---|
| `H2A_INSTANCE` | `remote:${SESSION_ID:-unknown}` | h2a identity. The `${SESSION_ID}` placeholder is resolved by remote's Pod template at Pod creation. |
| `H2A_HOST` | `remote` | Host hint reported to peers via the presence file. |
...
Example merge inside a `remote` session manifest (sketch):

```yaml
spec:
  containers:
    - name: runtime
      image: ghcr.io/remote/runtime:1.0.0
      # ... existing fields ...
      volumeMounts:
        - name: workspace
          mountPath: /workspace
    ...
    - name: h2a-mcp                # ← from the fragment
      ...
      env:
        - name: H2A_INSTANCE
          value: "remote:${SESSION_ID:-unknown}"
        - name: H2A_HOST
          value: "remote"
        - name: H2A_ROOT
          value: "/workspace/.h2a"
