...
- **AND** the note is not folded out by a **tombstone** — including the **edge cascade**: a note reachable only via a tombstoned node folds out too (recursive CTE + tombstone check, in this query — never a post-filter);
...
h-arch asked me to surface any technical reason SQLite cannot hold what graphify holds, rather than silently keep (a). I found none for `recallMemory`: graph traversal = recursive CTEs; vector = sqlite-vec; notes = a table; tenancy + tombstone-cascade + TTL + asOf are all expressible as WHERE/CTE inside the single query. **(b) holds for recall; no new owner decision is needed on this point.** (The one coupling that is *not* free — the D11 verdict-artifact FS-confinement store — is a separate first-rank invariant, out of scope for recall; see the companion note.)
...
- **AC5 — no second authority (structural).** No code path other than the recall query filters recall visibility by `scope`/`principal_owner`. A structural check (grep/lint) guards against a future re-implementation reintroducing the D10 dual-authority debt.
