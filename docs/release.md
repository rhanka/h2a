...
`.github/workflows/smoke.yml` runs on `workflow_dispatch` and on every push to `main`. It installs the currently published `@sentropic/h2a-cli@0.1.12` globally on a fresh Node 22 runner and exercises the published CLI surface:
...
`0.1.12` is the supported baseline for all installs.
