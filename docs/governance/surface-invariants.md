# The five h2a surface invariants ("6'")
...
control phrase known to be present returned **194**. The absence was measured, not assumed. The only
`I5` in the repository belongs to a different lane's document and states something else.
...
**I1 — one identifier crosses the boundary, and it is not a conversation id.**
...
**I2 — the surface states, operation by operation, what is MEASURED versus taken on trust.**
...
`[JUDGMENT]` I5 was first written as *"default = refuse"*. That version was a **remedy**, and it would
have forbidden a legitimate design — the dormant deposit-for-wake that eleven lanes rely on. The
reformulation above is a **principle**. The difference is not stylistic and it is the most useful
thing in this file: *an invariant written as a remedy forbids valid designs; written as a principle,
it sorts them.*

---

## 2. What I5 has actually caught

`[JUDGMENT]` The case for keeping these as principles rather than checklists is that a principle makes
a whole class **searchable**. Between 2026-08-01 and 2026-08-02, I5 surfaced four defects on four
...
`[JUDGMENT]` Case 2 matters most for reading I5 correctly: the invariant is **symmetric**. A tool that
...
## 3. Measured state, and where each measurement stops

`[MEASURED 2026-08-02, at publication]` I1 and I4 were re-measured on `origin/main` at the moment of
committing this file, because a table like this asserts the present tense:

- **I1 — violated.** `packages/h2a/src/runtime/identity/bindings.ts:46-50`: `IdentityBindingKey` is
  `{host, providerSessionId, workspaceId}`, and `findBinding` (:89-92) matches on `host` +
  `providerSessionId` only. `providerSessionId` changes per conversation, so a new conversation mints.
  **The key contains precisely what changes.** Repair belongs to WP5 (`runtime`), not to this file.
- **I4 — satisfied.** `packages/h2a/src/runtime/identity/live.ts:60` declares a single closed
...
`[SNAPSHOT 2026-08-01 — NOT re-measured today, and I say so rather than implying an audit]` I2, I3
and I5 were assessed on 2026-08-01 by `runtime` (WP5) and the plugins lane, with file and line cited
at the time: I2 satisfied at the sites traversed; I3 **partial** — the discipline exists but no
structural per-object marker does, so its rung is *habit*, not *structural*; I5 satisfied at the send
path under the corrected wording. Those three rows may have moved since. Anyone citing them should
re-measure first — that is the same rule this file applies to itself.
...
above are the whole set. There is no I6.
