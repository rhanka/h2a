...
| REC-07 | **One session carries two identities, and nothing joins them: the lane name is on one, the reachable inbox on the other.** *This entry replaces a wrong claim of this lane's own — see Q-06.* Measured end-to-end on this lane's own session, no third party involved: the single tmux session `h2a-memory` (one process, launched `--name memory`) is known to the **presence store** as `82afa836e60a` **named `memory`**, and to the **bus** as `b4ec36679141`, which is the identity that wakes and whose inbox is read. Consequences, each measured: (a) the presence store names all ten lanes correctly and `name` is absent from **0 of 71** files — so the field works, and the claim that presence lacks the launch name is false; (b) `h2a_discover_instances` returns **136** instances, `name` present in all, **42** distinct values, and **zero** lane names — it reads the bus registry, keyed on the other identity; (c) **a presence id accepts envelopes nobody will ever read.** Sending to `82afa836e60a` returned `ok: true, resolution: "deliver"` with `recipientLive: false`, while that presence file's heartbeat was **6 seconds old**, and the envelope landed in a separate inbox this lane is never woken for. **So never route to an id read from presence.** Had the ten live lane ids been handed over as targets, every message would have reported delivered and reached nobody. This is why `harness` could not reach `gateway` by name, and why a consultation once landed on an unrelated blocked third party. | measured here: presence store, `discover_instances`, and a self-addressed probe | **structural** | `h2a_discover_instances`; measured here | **structural** |
