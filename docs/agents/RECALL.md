...
own test would be the worst specimen of the class it documents. The one part with more than conventional force is the doctrine table's **gated invariant**:
`packages/h2a/test/actor-memory-wiring.test.js` fails if any table ULID is not a selected decision
in the journal, so a fabricated locator cannot pass CI. `--check` (byte-equality of the whole
projection) is a **generation-time** guard, not a CI gate — it is deliberately out of the gate
because CI runs the PR merged with main, whose journal is larger (REC-22). So the gate refuses a
fabricated citation; it does **not** refuse a hand-edited summary or a pointer buried in dead
prose — those stop at generation time and human review. The rest of this file has conventional force.
...
projection at generation time only. Nothing refuses a decision that no code obeys.
`graphify` made exactly this mistake with DOC-04 — citing a decided-but-unbuilt rule as the
cause of a real defect, which sends the fix to the wrong place.
...
| — | WP7 arbitration: what becomes of 'Infra, deploy & MCP'? | `B` — 8 leaves to their actor-memory owner: WP2 x3 (bus hygiene), WP5 x2 (cluster substrate, cgroups), WP9 (test bus isolation), WP12 (h2a mcp connector registration), WP14 (Terra preset). WP7 emptied now; cancelled only once reopen + code-release land. | — | `01KYQXRSCFPC8GQXSXNAF82YQ6` | 2026-07-29 |
| — | Greywall: posture par defaut du bac a sable autour des CLIs | `A` — Toute CLI demarre contrainte; une capacite refusee produit une erreur explicite et attribuable, jamais un resultat degrade. Escalade par declaration explicite, portee au run. | — | `01KYQYT3CA8VWK4CBHK1TDZXZH` | 2026-07-29 |
| — | C1: repatriate the two sentropic-exposure items under WP12 (WP8 produces the renderer, WP12 exposes it) | `A` — The two exposure-toward-sentropic items move to WP12; WP8 keeps producing the track report renderer, WP12 exposes it. | — | `01KYQYZ6X2X8BJKWH6Y21116TX` | 2026-07-29 |
| DOC-07 | Where does durable actor memory live, now that git ignores it? | `A` — RECALL.md and DELEGATION.md already live there; the briefs and COMMON.md join them, and the launch path is repointed in the same change. | **Durable actor memory lives at the tracked path `docs/agents/`, and only the durable part.** `tmp/` stays ignored — the test runner writes `tmp/test-runtime` and worktrees live under `tmp/worktrees`, so un-ignoring it would version scratch. Tracked: `COMMON.md`, the twelve actor briefs, `RECALL.md`, `DELEGATION.md`, `launch.sh`. **Deliberately NOT tracked: per-task subcontract briefs** — they are written for one delegation and die with it, and versioning them would turn a working directory into a graveyard. The line is durability, not importance. Short-name aliases (`BRIEF-arch.md`, `BRIEF-cond.md`) were byte-identical copies of the canonical files and are not carried over: one file per actor, or the copies drift. Its decision now resolves on `origin/main` (recovered 2026-07-31); this row promotes from legacy to the projected table at the next regeneration against main — see REC-12. | `01KYQZXCEZJXYAJ04YB5YMEWK0` | 2026-07-29 |
| — | Fallback inter-pools et cible Gemini sur quota epuise : capacite retiree ou retablie | `A` — La passerelle refuse explicitement quand le pool colle est epuise, au lieu de rebasculer vers un autre pool ou vers une cible Gemini. Les deux tests qui exigeaient la bascule sont reecrits pour affirmer le refus. | — | `01KYR0WDVTYT7320NXS51X77CQ` | 2026-07-29 |
| — | Comment H2A doit-il éviter d’envoyer une commande à la mauvaise session ? | `A` — Aucune commande ne part sans une cible resolue, unique et dont la disponibilite est etablie. | — | `01KY65RNV7GWCXV787PJW5FYQ1` | 2026-07-30 |
| — | 1/6 — Un nom de session peut-il envoyer une commande ? | `A` — Un nom sert à retrouver et afficher une session ; commander exige une cible vérifiée. | — | `01KY66FTP7Y4W214SADDWKHT15` | 2026-07-30 |
| — | 2/6 — Après suppression, peut-on réutiliser le même nom ? | `B` — Le nom redevient disponible une fois la fenêtre écoulée. | — | `01KY66FV4W1JKHBZ2MW44P9EP6` | 2026-07-30 |
| — | 5/6 — H2A peut-il écrire dans un terminal ambigu ? | `A` — Terminal ambigu : h2a n écrit pas. | — | `01KY66FWJ4MYNJMDDJXA54G3FD` | 2026-07-30 |
| — | 6/6 — Comment lancer le moteur natif H2A ? | `A` — h2a possede la boucle et la lance en local ; sentropic reste consommateur. | — | `01KY66FX0PFH958103YVQWRBQZ` | 2026-07-31 |
| — | Release 0.86.0: publish it, then update the owner installation including the plugins | `A` — npm publie 0.88.0; la release est depassee deux fois. Residu (install owner) = defaut vivant WP10, pas une question. | — | `01KYJ4JNCCRG5DSFXR3N3Z7826` | 2026-07-31 |
| — | main gets a required check, but only one that can judge a PR on its own code | `A` — build-and-test node 20/22 + security-debt + smoke-install x3, enforce_admins TRUE. La dette est payee. | — | `01KYJ4JNMSDKHZBEW79RHK1DSG` | 2026-07-31 |
| — | Canonical h2a root: two live roots do not see each other, and it now blocks the delegation mandate itself | `A` — Remove --root from ~/.claude.json so every host resolves the same default. The pinned store becomes legacy. | — | `01KYRBCJRKND9WWJSD0VSZH7BT` | 2026-07-31 |
| — | The twelve-subcontractor cap was breached by 58 percent within three hours and nothing could refuse, detect or attribute it | `C` — No mechanism, no promise. The number is guidance, the hourly count is the only control, and the record says the guarantee stops there. | — | `01KYRE9F4G0TFD7CT1G1VRS4SF` | 2026-07-31 |
| — | PR 90 rebase bloque par un merge-commit interne, main ne touche aucun de ses fichiers: merge-commit, reecriture, ou attente | `A` — memory a fusionne origin/main DANS la branche: base a jour via merge-commit, ni rebase ni squash, zero conflit (main orthogonal aux 20 fichiers, verifie). Contenu revu BYTE-IDENTIQUE 647e5ef5->3ff11c48 (0 des 20 fichiers modifie, verifie par moi). L ecart a la regle rebase DISPARAIT: la base est a jour, retard=0. Reste a merger quand CI verte sur 3ff11c48. memory rapporte un go owner sur ce chemin (non temoigne par moi). | — | `01KYVJ2JC9VT014XS6TPWD72S9` | 2026-07-31 |
...
| REF-04 | *A live PID proves an agent is working.* | One lane showed 1 s of CPU over 33 min. Liveness is CPU time, not the PID. Source: `COMMON.md` (now tracked under `docs/agents/`). | habit. **Not re-measured by this memory.** |
...
| REC-21 | **Missing memory does not only cause repeated work — it hides systemic defects behind independent "isolated" verdicts.** Measured by `graphify` on 2026-07-30, running nine roles in one workspace: two roles each found a NUL-byte-in-source defect, each concluded it was an isolated slip, and neither could see the other's finding; their conductor scanned and found it **systemic — 6 files on main**. Separately, three roles independently rediscovered that `/rename` is a host built-in no agent can type from inside its own session, each paying the discovery cost. **The second case is the sharper argument for this lane than anything measured inside h2a:** shared memory does not merely stop rework, it **aggregates independent partial observations into one finding**. N agents each concluding "isolated" is the signature of a systemic defect nobody can see. *(relayed by `graphify`, not re-measured here — their repo, not this one.)* | `graphify`'s measurement | **structural** (the loss), habit (the fix) |
| REC-22 | **A local red is exactly as untrustworthy as a local green — only the deciding configuration decides.** Both directions were measured on 2026-07-30. `plugins` measured **1 706 pass / 0 fail locally against 1 684 / 22 FAIL in CI at the same SHA**, because a test that consults `PATH`, `HOME`, an installed CLI, a clock or the network passes on the author's machine and nowhere else. **This lane produced the mirror image and reported it twice as fact:** its subcontractor could not run the root gate — runtime compilation errors in its worktree — and this lane relayed "the root gate does not pass on my branch" to the conductor and to the owner. It does pass: workflow `ci`, `conclusion: success`, `headSha` matching `HEAD` exactly, `build-and-test` on Node 20 and 22 plus `smoke-install` on three platforms. A local failure is an observation about a machine, not about a branch. **Cite the configuration and the SHA, never "it passes" or "it fails" alone.** And a specific CI-only trap this lane then walked into: **CI runs a PR on the PR-MERGED-WITH-MAIN tree** (`refs/pull/N/merge`), so any test that compares a committed artefact to a shared, advancing source — e.g. byte-equality of a generated table against `.track/events.jsonl` — is green on the branch and RED in CI the moment `main` advances, because the merged journal is larger. Gate the invariant (does each locator resolve), never the exact projection. | `plugins`' measurement *(relayed)*; this lane's own false report, corrected against `gh run view` at `55db8896` | **structural** |
...
| REC-07 | **Presence and addressing live in different stores on different paths — it is a PARTITION, not a badly chosen key.** *Second correction of this entry; the first replaced a wrong claim of this lane's own (Q-06).* **Two disjoint bus roots each mint a different uuid for the same tmux session, at the same instant.** Found by `graphify` re-running this lane's measurement across nine roles; reproduced here read-only on both roots: `/home/antoinefa/src/a2a-cli` holds 59 presences, **9** channel identities `claude:graphify-*` and **10** channel inboxes, while `/home/antoinefa/h2a-workspace/.h2a` holds 138 presences, **0** channel identities and **0** channel inboxes — their `knowledge` role being `claude:graphify:32ae534a470e` in one root and `claude:graphify:d613c9cb8c9e` in the other, both live. **An agent can therefore be perfectly reachable and totally invisible at once**, depending on which root its `mcp-serve` started with, and a name absent from the root you are reading is not an absent name. The same shape was first measured here on one session: the tmux session `h2a-memory` is `82afa836e60a` named `memory` to the presence store and `b4ec36679141` to the bus that wakes it, and an envelope sent to the presence identity returned `ok: true, resolution: "deliver"` with `recipientLive: false` against a 6-second-old heartbeat, landing in an inbox this lane is never woken for. **Consequences: never route to an id read from presence; a key containing an instance uuid is not merely ephemeral but ambiguous at the same instant; and verify every send by READING the recipient's inbox file** — which is how a consensus request was confirmed to reach `claude:graphify-knowledge` in root A while this lane's presence reads see root B. | both roots measured here; the nine-role partition measured by `graphify` | **structural** |
...
| REC-18 | **The coordination channel fails under the load it exists to carry.** `h2a_inbox read` now returns **431 000 characters** — past the token ceiling, so the conductor cannot open its own inbox. There is no pagination and no filter. Measured the same night: `h2a_discover_instances` returns 70 609 characters, likewise unreadable in one call. The surface degrades precisely when twelve lanes make coordination matter most, and it degrades **silently** — the call succeeds, the reader just cannot receive it. Same surface, second symptom, measured here on a cross-project send: addressing `claude:graphify-conductor` returned `resolution: "deliver-hint"`, `recipientConfidence: "idle-uncertain"`, and the advice *"prefer re-sending to `claude:graphify-conductor` for direct delivery"* — **the hint names the address that was just used**, so following it loops. The envelope *was* written (verified by reading the channel inbox: 22 envelopes, mine among them), so the advisory layer was wrong about its own outcome. **Consequence for this lane: any diffusion path that assumes an actor can read its inbox is unreliable, and the bus's own confidence field cannot be taken as the outcome — read the recipient's inbox instead. That is a second argument for putting standing rules in a replaying prompt rather than a message.** Not this lane's to fix — protocol and bus are `coop` (WP1-3). | `conductor`'s measurement; `discover_instances` measured here | **structural** |
| REC-19 | **A cap no mechanism refuses is not a cap, it is an intention — and it is exceeded exactly when it matters.** The owner set twelve parallel subcontractors on 2026-07-29 and made `conductor` accountable for it. `conductor` said at its first iteration that the cap was held **by hand** and that nothing would refuse a thirteenth. Three hours later: **19 alive**, seven of them appearing inside 22 minutes, at 322 node/codex processes and 34.7 GB resident with 17 GB free of 57 — on a machine that has already taken OOM kills. It could not prevent it, could not attribute it (the launch name is on an identity the bus does not use — REC-07), and could not detect it before the next iteration. **What would falsify this entry: a launch the tool refuses past the threshold.** The guarantee stops at counting. | `conductor`'s measurement *(relayed)*; this lane returned its own subcontractor's 265 MB on request and did not replace it | **habit** — and named as one |
...
| Q-06 | **"The registry carries no field that discriminates one actor from another, and no lane name is registered."** — written by **this lane**, in this file, and relied on by others for six hours. | `agents` read the presence files instead of grepping them: `name` absent in **0 of 67**. `conductor` reproduced it on 73. This lane then reproduced it independently on 71 files — 0 absent, 33 distinct values, and **all ten lane names present**, each with a heartbeat seconds old. The original claim came from `h2a_discover_instances` alone and generalised "32 instances share the value `h2a`" into "no field discriminates". | **refuted, and it was mine.** Two errors, not one: the field exists, and the lanes *are* named — just on an identity the bus does not use (REC-07). A dossier that only quarantined other people's claims would be a partial dossier; the author's own belongs here on the same terms. |
