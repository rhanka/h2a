# Instruction note — h2a on Kubernetes + interop with `@sentropic/remote-controle`

> Status: **research / design proposal, not implementation**. Sourced 2026-05-23 from `../poc-k8s` and `../remote` (`@sentropic/remote-controle`).
...
### `../remote` (`@sentropic/remote-controle`)
...
- `remote-controle` runs **one CLI inside k8s** and gives it a workspace + terminal + browser.
- `h2a` runs **multiple CLIs that talk to each other** through shared state.

If a `remote-controle` session also opens an h2a session, the operator UI gets to see *who else is on the bus* and *what messages were exchanged*, on top of seeing one isolated terminal stream. That's the value of the interop.

## Three concrete deployment scenarios

### Scenario A — h2a `mcp-serve` as a sidecar in a `remote-controle` session Pod
...
- All sessions in the same `remote-controle` workspace are visible to each other through the local `.h2a/` mount.

Pros: trivial to ship (one sidecar definition). No cross-Pod traffic. Reuses existing PVC.
Cons: peers across different `remote-controle` workspaces are NOT visible. h2a stays per-workspace.
...
- `remote-controle` session Pods opt-in by mounting the same PVC (ReadWriteMany — Scaleway block storage doesn't support this natively, would need NFS-backed PVC or a small dedicated `h2a-store` Pod exposing the filesystem over a synced protocol).
...
- `remote-controle` control plane hosts a single `h2a-broker` Deployment exposed via the existing traefik ingress.
...
## What an interop contract with `remote-controle` would say

A draft `CONTRACT` artefact between `@sentropic/h2a` and `@sentropic/remote-controle`:

1. **Identity bridge** — when `remote-controle` creates a session with `SessionDescriptor.id = X`, the in-Pod `mcp-serve` MUST open an `H2ASession` with `instance = "remote-controle:" + X` and `host = profile.cli`.
...
4. **Disclosure** — the operator UI of `remote-controle` MAY consume h2a's `h2a_discover_sessions` to enrich the session list with peer presence, but MUST NOT bypass `h2a_session_open` (i.e. presence and registry are h2a-owned).
5. **Auth boundary** — until V2 transport auth lands, the shared `<root>` filesystem trust boundary is the **`remote-controle` workspace boundary**. Cross-workspace cooperation requires Scenario C.

This belongs in `@sentropic/remote-controle`'s `packages/protocol` as a new schema (an `h2aBridgeSchema`) and in `@sentropic/h2a`'s `DECISIONS.md` as a sibling DEC once we commit.
...
h2a deploy --target remote-controle --session <id> [--mode sidecar|peer]
...
- A `docs/contracts/remote-controle-bridge.md` documenting the identity bridge contract.
...
3. Should the interop contract with `remote-controle` be one-way (h2a documents the expectations) or two-way (both repos commit to the same schema)?
