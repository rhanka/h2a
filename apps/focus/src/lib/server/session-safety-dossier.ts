...
export interface SessionSafetyDecision {
  key: string;
  question: string;
  whyNow: string;
...
export interface SessionSafetyDossier {
  revision: string;
  title: string;
  context: string;
  decisions: SessionSafetyDecision[];
}

// Source locale d'autorité du dossier. Ces choix sont préparatoires : ils sont
// destinés à être inclus dans une CLI live, jamais enregistrés comme décision
// Track par cette route Focus.
export const sessionSafetyDossier: SessionSafetyDossier = {
  revision: 'session-safety-2026-07-23',
  title: 'Dossier de décisions — sécurité des sessions',
  context:
    'Après « h2a run claude --name a20 », le nom humain, la session tmux, l’instance de protocole, l’attachement live, la conversation fournisseur et le rôle de boucle peuvent tous désigner des choses différentes. Ces six arbitrages définissent les règles sûres avant toute implémentation.',
  decisions: [
    {
      key: 'D1',
      question: 'À quel moment h2a run doit-il prouver l’adressabilité du protocole ?',
      whyNow:
        'Un lancement simple peut ne créer aucune présence de protocole. Une boucle structurée ne peut pas annoncer un succès ni superviser un processus sans adresse vérifiée.',
      options: [
        {
        ...
        },
        {
          ...
          behavior:
            'Les boucles objectif et lancements structurés exigent une adresse vérifiée. Un lancement interactif peut continuer uniquement avec un résultat explicitement dégradé.',
          consequence:
            'L’automatisation ne surinterprète jamais sa supervision, tandis que la personne conserve un chemin de récupération clairement annoncé.',
          recommended: true
        },
        {
      ...
      recommendation:
        'Exiger l’adressabilité vérifiée pour les lancements structurés et les boucles objectif ; autoriser un lancement interactif explicitement dégradé.',
    ...
    },
    {
      ...
      whyNow:
        'Le nom de lancement `--name` peut correspondre au `/rename` visible dans Claude, sans en être l’identité. La réponse détermine ce que promet « a20 », comment les collisions apparaissent et si un lancement ultérieur peut le réutiliser sans risque.',
      options: [
        {
          key: 'espace-travail',
        ...
        },
        {
          ...
          behavior:
            'Conserver un nom de lancement actif dans cet espace de noms, garder l’historique immuable des générations de lancement et refuser les collisions inter-sources.',
          consequence:
            'Le nom de lancement reste utile à l’opérateur sans devenir une adresse de protocole durable ni sélectionner silencieusement la mauvaise session.',
          recommended: true
        },
        {
      ...
      recommendation:
        'Utiliser un nom de lancement actif par espace de noms runtime / serveur tmux, conserver les générations immuables et fermer les collisions.',
    ...
    },
    {
      ...
      whyNow:
        'Un alias est un sélecteur humain, pas un destinataire de protocole. Le traiter comme une route peut envoyer du travail au mauvais principal.',
      options: [
        {
          key: 'instance-exacte',
        ...
        },
        {
          ...
          behavior:
            'La livraison non interactive exige toujours l’instance complète. L’interactif peut utiliser un alias résolu de façon unique avec intention de canal explicite et sortie auditable.',
          consequence:
            'La frontière de protocole reste intacte tout en offrant un confort limité qui refuse toute ambiguïté.',
          recommended: true
        },
        {
          key: 'alias-ordinaire',
      ...
      recommendation:
        'Conserver l’instance complète pour toute livraison non interactive ; ne permettre qu’un confort d’alias interactif, opt-in, unique et auditable.',
    ...
    },
    {
      ...
      whyNow:
        'C’est l’arbitrage le plus déterminant : il contrôle les clés, la reprise fournisseur, les rôles, la consommation d’inbox et le sens des attachements live.',
      options: [
        {
          key: 'hote-fournisseur',
          title: 'Acteur hôte intégré au fournisseur ; lanceur comme proxy de réveil',
          behavior:
            'Quand la preuve fournisseur et clé existe, l’hôte intégré MCP possède l’acteur. Un sidecar lanceur frère ne porte que le contexte de réveil vérifié sans transfert authentifié.',
          consequence:
            'L’acteur est aligné sur la continuité fournisseur et la preuve de clé sans laisser un auxiliaire de lancement revendiquer silencieusement l’autorité.',
          recommended: true
        },
        {
          key: 'sidecar-lanceur',
      ...
      recommendation:
        'Préférer l’acteur hôte intégré au fournisseur quand la preuve fournisseur / clé existe ; conserver le sidecar comme proxy de réveil sans transfert authentifié.',
    ...
    },
    {
      ...
      whyNow:
        'Le protocole permet des attachements concurrents. Choisir le terminal le plus récent ou assimiler présence et acteur peut réveiller le mauvais terminal.',
      options: [
        {
          key: 'plus-recent',
        ...
        },
        {
          ...
          behavior:
            'L’acteur pérenne conserve son inbox. Un réveil de terminal n’est permis que lorsqu’un attachement de lancement est vérifié de façon unique ; sinon il est refusé.',
          consequence:
            'L’identité pérenne est préservée sans inventer une règle de sélection de terminal lorsque les preuves sont ambiguës.',
          recommended: true
        }
      ],
      recommendation:
        'Conserver une inbox d’acteur pérenne et ne réveiller un terminal qu’au moyen d’un attachement de lancement vérifié de façon unique.',
    ...
    },
    {
      ...
      whyNow:
        'Les stores runtime et protocole ont des propriétaires, rétentions et réparations distincts. Cacher la liaison dans l’un des deux risque une réparation dernier-écrivain-gagnant.',
      options: [
        {
        ...
        },
        {
          ...
          behavior:
            'Un index dédié référence les enregistrements runtime et protocole natifs sans remplacer leur source de vérité.',
          consequence:
            'Les preuves inter-propriétaires et les échecs partiels restent visibles, et une génération obsolète ne peut pas écraser un lancement plus récent.',
          recommended: true
        }
      ],
      recommendation:
        'Utiliser un index de lancement neutre, additif et par génération, qui référence sans remplacer les stores runtime et protocole.',
...
export function loadSessionSafetyDossier(): SessionSafetyDossier {
  return sessionSafetyDossier;
}

export function findSessionSafetyDecision(key: string): SessionSafetyDecision | undefined {
  return sessionSafetyDossier.decisions.find((decision) => decision.key === key);
}

export function findSessionSafetyOption(decision: SessionSafetyDecision, key: string): DossierOption | undefined {
