export interface DossierOption {
  ...
  consequence: string;
  /**
   * NEVER set in this dossier — it is neutral by construction (UAT A2). The field is kept so the
   * renderer stays generic for other dossiers; an agent-memory option carrying it is a defect.
   */
  recommended?: boolean;
}

/**
 * One mechanism-level fact, placed on the card that needs it rather than in an appendix.
 *
 * `status: 'unverified'` is load-bearing: the source research marks several claims as UNVERIFIED, and a
 * decision card that presents an unverified claim as fact is worse than one that omits it. The most
 * valuable line in a decision card is an honest "we could not establish this".
 */
export interface MechanismFact {
  /** The system the fact is about — the reader needs to know whose mechanism this is. */
  system: string;
  fact: string;
  /** Research section + file:line / issue / arXiv id, so the claim is checkable, not just asserted. */
  source: string;
  status?: 'unverified';
}
...
export interface AgentMemoryDecision {
  ...
  /** Neutral by contract: names the criterion to weigh, never a pick. Rendered as "critère à trancher". */
  recommendation: string;
  nextWork: string;
  /** Round-1 decisions this card derives from. Absent on the original seven. */
  parent?: string[];
  /** The owner's own words that generated this card. Verbatim, French, never paraphrased. */
  fromAnswer?: string;
  mechanisms?: MechanismFact[];
  /** What could not be established. Rendered as such, never smoothed into prose that implies certainty. */
  unknowns?: string[];
  /** Revision in which this card first appeared, so a reader can tell round 1 from round 2. */
  addedInRevision?: string;
}

/**
 * What survives a revision bump.
 *
 * Bumping the revision must not orphan the committed answer set. The honest way is not to guess: it is to
 * state, in the fixture itself, which decision keys were kept identical (so they replay in full) and which
 * are new (so they have no committed answer yet). The page renders this next to the replay control.
 */
export interface RevisionCarryOver {
  from: string;
  /** Kept identical — same decision key AND same option keys — therefore fully replayable. */
  carried: string[];
  /** Added by this revision: no committed answer exists for these yet. */
  added: string[];
  statement: string;
}

/** A claim the previous revision presented as fact and that the mechanism research corrected. */
...
// Source locale d'autorité de ce dossier. Révision 2 : elle répond au reproche explicite du propriétaire
// sur la révision 1 (« tu ne m'as pas présenté comment hermes fait ou d'autres, il faut plus de détail »)
// en plaçant les faits de MÉCANISME sur la carte qui en a besoin, et elle transforme ses propres réponses
// en nouvelles cartes. Elle ne recommande aucune option et n'est jamais transmise automatiquement à une
// CLI live ni enregistrée comme décision Track par cette route Focus.
  ...
  corrections: [
    {
      ...
      actually:
        'Les plafonds sont en CARACTÈRES, pas en tokens : memory_char_limit = 2200, user_char_limit = 1375, surchargeables dans config.yaml. Les chiffres en tokens sont dérivés, jamais appliqués.',
  ...
  decisions: [
    {
      ...
      recommendation:
        'Trancher d’abord si l’agent doit avant tout SAVOIR (corpus stable interrogeable) ou SE SOUVENIR (ce qui s’est dit et fait en session) — et, au vu des mécanismes ci-dessus, peser un point que la révision 1 masquait : les deux archétypes ne diffèrent pas seulement par ce qu’ils stockent, mais par le moment où l’écriture devient visible (instantané gelé jusqu’à la session suivante chez Hermes, immédiat chez Letta, à la reconstruction chez graphify).',
      nextWork:
        'Lister les sources déjà disponibles (corpus documentaire graphify, historiques de session existants, adaptateur de conversations) et vérifier lesquelles sont réellement exploitables avant de choisir une couche.',
    },
    {
      key: 'D2',
      ...
      options: [
        {
          ...
          behavior:
            'Entités et relations extraites par LLM dans un graphe avec résolution d’entités et validité bi-temporelle. Chez Graphiti, add_episode enchaîne : validation des types, résolution du group_id, récupération de l’épisode précédent, extraction des nœuds (1 appel LLM), résolution des nœuds extraits (recherche par embedding, puis nom exact, puis MinHash/Jaccard ≥ 0,9, puis repli LLM par lots), extraction des arêtes (1 appel LLM), résolution des arêtes, attributs et résumés, puis UNE seule écriture en masse.',
          consequence:
            'Réconciliation la plus fidèle du corpus. Le coût réel, dérivé de la lecture du code et documenté NULLE PART ailleurs — pas même dans l’article : environ 2 + 3E + N + 2 appels LLM par épisode, donc LINÉAIRE EN NOMBRE D’ARÊTES, pas en O(1). Il faut aussi un serveur de graphe, et le budget de types est plafonné à 10 types d’entités et 10 types d’arêtes, ≤ 10 champs chacun.',
        },
        {
          key: 'graphe-ontologique-corpus',
          title: 'Graphe ontologique de corpus',
          behavior:
            'Un pipeline batch transforme un corpus en graphe typé dédupliqué avec entités canoniques. graphify : JSON graphology avec citations, evidence_refs, confidence_handle, provenance_handle et un cycle de vie de revue à 9 états ; déjà uni-temporel (t, t_end, t_iso, t_src) avec recall --as-of. cognee : l’ontologie n’est PAS une contrainte de prompt mais un ancrage POST-extraction — l’extraction tourne d’abord sans contrainte, le LLM ne voit jamais les classes de l’ontologie dans le chemin par défaut, puis un résolveur apparie par chaîne floue (difflib, cutoff 0,8) et renomme vers le nom canonique.',
          consequence:
            'Structure la plus auditable et token-efficiente pour un savoir stable. Mais elle est pensée pour un corpus construit par lots, et sa temporalité est à UN seul axe : elle sait « quand le fait valait », pas « quand nous l’avons appris ». Ajouter le second axe est la question de la carte D9.',
        },
        {
          key: 'vecteurs-auto-extraits',
          title: 'Vecteurs sémantiques auto-extraits',
          behavior:
            'Un LLM extrait des faits, les embed et les indexe pour un rappel par similarité — mem0 (Qdrant/pgvector/Chroma/LanceDB pluggables), LangMem (Postgres+pgvector), LanceDB embarqué. À noter côté LangGraph : le SummarizationNode n’écrit explicitement PAS dans le magasin long terme ; le vrai chemin d’écriture de LangMem est ReflectionExecutor, une tâche de fond débouncée après une pause de conversation.',
          consequence:
        ...
        },
        {
          ...
          behavior:
            'La mémoire vit en fichiers texte explicitement écrits, sans vecteurs ni index. ctx lit .context/ dans un ordre de priorité fixe (CONSTITUTION.md, TASKS.md, CONVENTIONS.md, ARCHITECTURE.md, DECISIONS.md, LEARNINGS.md, GLOSSARY.md, AGENT_PLAYBOOK.md) et sert un paquet de contexte par allocation scorée sous budget. L’outil memory d’Anthropic expose un dossier virtuel /memories dont VOUS possédez les octets.',
          consequence:
            'Portabilité et transparence maximales, RAM minimale, aucun rappel sémantique. Le risque n’est pas technique mais humain : c’est l’archétype où la dégradation est la plus silencieuse (voir les issues de troncature et d’accumulation périmée de Hermes, carte D1).',
        },
        {
          key: 'memoire-hierarchique-auto-editee',
          title: 'Mémoire hiérarchique auto-éditée',
          behavior:
            'Un petit bloc toujours en contexte que l’agent édite lui-même, avec débordement vers un stockage archivé. Letta rend ses blocs en XML en exposant au modèle son propre budget (chars_current / chars_limit), et offre une famille d’outils d’édition de plus en plus précise : core_memory_append/replace historiques, puis memory_replace (appariement unique, refuse les chaînes préfixées d’un numéro de ligne), memory_insert, memory_rethink, jusqu’à memory_apply_patch, un diff unifié à la Codex appliqué côté serveur.',
          consequence:
            'L’état saillant est toujours disponible. Mais chez Letta le champ limit semble PUREMENT INDICATIF — aucune application n’a été trouvée dans le chemin d’écriture : update_block_value() ne vérifie que le type, et la docstring de la mise à jour en masse PRÉTEND lever au-delà de la limite alors que l’implémentation est une simple affectation. C’est l’exact inverse de Hermes, qui échoue en fermeture.',
        },
      ],
      mechanisms: [
        {
        ...
        },
        {
          system: 'cognee',
          fact:
            'La temporalité est native mais NON bi-temporelle : Timestamp, Interval(starts_at, ends_at) et Event forment des intervalles d’événement sur une seule ligne de temps — pas de temps de transaction, pas d’invalidation pilotée par contradiction. Le trio de stockage est SQLite + LanceDB + Ladybug, et Ladybug vient tout juste d’atterrir : le mainteneur écrit lui-même qu’il est « plus simple pour l’instant d’utiliser Kuzu en expérimentation locale » — à épingler en version.',
      ...
      recommendation:
        'Peser le besoin de structure relationnelle et de temporalité contre le coût d’opération, en tenant compte d’un chiffre que la révision 1 n’avait pas : la structure la plus fidèle coûte un nombre d’appels LLM linéaire en nombre d’arêtes par épisode, et ce coût n’est écrit nulle part dans sa documentation.',
      nextWork:
        'Prototyper un rappel sur un échantillon réel de requêtes de l’agent pour chaque structure candidate et comparer précision, latence et RAM — et, avant tout arbitrage temporel, lever l’incohérence d’intervalle fermé/demi-ouvert déjà présente dans graphify.',
    },
    {
      key: 'D3',
      question: 'Qui décide qu’un fait entre en mémoire : une extraction automatique ou une écriture curée et revue ?',
      whyNow:
        'Multi-session, multi-CLI signifie que plusieurs agents écriront dans la même mémoire sans supervision continue de l’opérateur — il faut décider avant tout déploiement si chaque écriture doit être explicite ou relue, ou si un extracteur automatique peut agir seul. Deux publications récentes rendent la question plus dure qu’elle n’en avait l’air : voir les mécanismes.',
      options: [
        {
          key: 'auto-extraction',
          title: 'Extraction automatique par le LLM',
          behavior:
            'Un extracteur LLM décide seul quoi retenir et écrit à chaque tour ou épisode — mem0 (routeur ADD/UPDATE/DELETE/NOOP), Graphiti (add_episode), Letta (auto plus auto-édition). Chez Letta le déclencheur du travail de fond est un compteur de tours persisté modulo N : on incrémente, puis on tire quand le reste est nul.',
          consequence:
            'Couverture large sans effort opérateur, mais chaque écriture dépend du jugement du modèle. Deux résultats de 2026 chiffrent ce risque, et le second porte précisément sur une boucle autonome — voir les mécanismes de cette carte.',
        },
        {
          key: 'ecriture-gatee',
          title: 'Extraction automatique avec porte d’approbation',
          behavior:
            'Le LLM propose une écriture, une validation peut la bloquer avant qu’elle n’entre dans le substrat. Chez Hermes, memory.write_approval est à false PAR DÉFAUT ; activée, elle approuve en ligne au premier plan mais MET EN ATTENTE tout le reste — messagerie, cron, scripts, relecteur de fond — dans pending/memory/<id>.json, géré par /memory pending | approve | reject. Les skills mettent TOUJOURS en attente. Les écritures automatiques sont étiquetées [auto].',
          consequence:
            'Conserve la couverture automatique tout en ouvrant un point de contrôle. Deux propriétés de la seule implémentation vraiment livrée méritent d’être vues avant de s’en inspirer : il n’existe AUCUN état « refuser d’emblée » — la porte ne fait que RETARDER ; et elle est désactivée par défaut.',
        },
        {
          key: 'curee-revue',
          title: 'Écriture explicite et curée',
          behavior:
            'Seules des commandes explicites déposent un fait, avec revue humaine possible. graphify offre la surface de revue la plus solide du corpus : file de candidats, validate_ontology_patch puis apply_ontology_patch, dry-run par défaut, journal de décisions autoritaire plus un audit JSONL, cycle de vie à 9 états, chemin mis en prison, et une exposition MCP derrière un drapeau d’écriture explicite. La file est estampillée graph_hash, profile_hash et generated_at pour que la péremption soit détectable.',
          consequence:
            'Précision et auditabilité maximales, aucune dérive silencieuse, mais couverture partielle. Et une propriété de conception à connaître : ce cycle de vie a été bâti pour des candidats par LOTS ; la capture continue inverse le rapport — c’est l’objet de la carte D11.',
        },
      ],
      mechanisms: [
        {
    ...
    },
    {
      ...
      options: [
        ...
        },
        {
          ...
          consequence:
            'Résout le doublon et le désaccord le plus courant à faible coût, mais perd la trace de ce qui a changé et quand, sauf là où un historique explicite existe.',
        },
        {
          key: 'aucune',
          title: 'Aucune réconciliation',
          behavior:
            'Chaque écriture s’ajoute sans fusion ni détection de conflit — RAG vectoriel brut (pgvector/Chroma/LanceDB/txtai), fichiers curés tant que l’humain ne les édite pas. À signaler comme astuce transposable : cognee obtient une convergence sans coordination par des identifiants DÉTERMINISTES (id_for() sur des identity_fields), donc une écriture répétée fusionne par identité au lieu de dupliquer.',
          consequence:
            'Substrat le plus simple à opérer, mais les faits contradictoires coexistent silencieusement et la précision du rappel se dégrade avec le volume.',
        },
      ],
      mechanisms: [
        {
      ...
      recommendation:
        'Décider si répondre à « que savait l’agent à la date T, et pourquoi a-t-il changé d’avis » a de la valeur ici — et, si la réponse est oui et que le substrat retenu est graphify, mesurer que ce qui manque n’est pas le champ temporel (il est presque gratuit) mais le DÉTECTEUR de contradiction, qui est un algorithme différent de celui que graphify possède.',
      nextWork:
        'Construire 3 à 5 scénarios de contradiction réels (changement de préférence, correction d’un fait) et vérifier comment chaque stratégie les traite, en distinguant explicitement réconciliation d’identité et réconciliation d’assertion.',
    },
    {
      key: 'D5',
      question: 'Où héberger la mémoire sous la contrainte RAM/OOM : embarqué local, serveur+base de données, ou SaaS ?',
      whyNow:
        'La contrainte RAM/OOM est déjà vécue ailleurs dans ce projet. Aucun système du benchmark ne publie de chiffres RAM : le budget disponible doit être fixé et mesuré avant, pas après, avoir committé une architecture serveur+DB. Cette révision apporte enfin UNE mesure, prise localement — voir les mécanismes.',
      options: [
        {
          key: 'embarque-local',
          title: 'Embarqué, local-first',
          behavior:
            'Bibliothèque intégrée au process de l’agent, sans serveur séparé — cognee en mode par défaut, LanceDB (fichier disque, gère des jeux plus grands que la RAM), mem0 en mode librairie, ctx (fichiers), graphify (JSON local), txtai.',
          consequence:
            'Empreinte la plus faible et déploiement le plus simple. Mais la mesure prise sur ce poste donne le plancher réel du modèle actuel de graphify : lecture 95 ms, parsing 161 ms, sérialisation 246 ms, écriture 118 ms, soit environ 620 ms à 267 Mo de tas et 373 Mo de RSS pour UN aller-retour de graphe complet, AVANT toute extraction. Au vu de l’historique d’OOM de cet espace de travail, une écriture par tour est une décision de gouvernance des ressources, pas un détail.',
        },
        {
          key: 'serveur-plus-bd',
          title: 'Serveur applicatif + base de données',
          behavior:
            'Un serveur dédié porte la mémoire, les sessions s’y connectent en client — Letta (serveur + Postgres/pgvector), Zep/Graphiti (Neo4j ou FalkorDB), mem0 en mode serveur. Côté graphify, le port GraphStore expose déjà quatre implémentations : file, neo4j, spanner, postgres.',
          consequence:
            'State partagé nativement entre process et sessions. Mais la disparité de capacités entre backends n’est pas cosmétique : queryWindow — le chemin de LECTURE de recall --as-of, donc de tout ce qui est temporel — n’existe QUE sur postgres. Choisir neo4j pour le web, c’est perdre la lecture temporelle à la couche de stockage.',
        },
        {
          key: 'saas',
      ...
      mechanisms: [
        {
          ...
          fact:
            'C’est une UNION 2-VOIES À CROISSANCE SEULE, pas une fusion 3-voies : l’ancêtre est bien lu, mais uniquement dans un garde-fou — la fusion porte sur [courant, autre]. Conséquence structurelle : LES SUPPRESSIONS NE SE PROPAGENT PAS. Un nœud supprimé sur une branche est RESSUSCITÉ par l’autre (le cas d’école du G-Set, ou d’un 2P-Set sans pierres tombales). Donc « git gère la concurrence » est vrai pour les ajouts et FAUX pour les retraits : une mémoire corrigée ou oubliée sur une branche revient par l’autre.',
          source: 'recherche §5 — src/merge-driver.ts',
        },
        {
          system: 'graphify — le pilote de fusion',
          fact:
            'Deux politiques de conflit COHABITENT dans le même fichier : les attributs scalaires de nœuds sont en dernier-écrivain-gagnant avec « theirs » qui gagne (l’ordre de boucle est [courant, autre] et le mergeNode de graphology est un LWW peu profond), tandis que les ARÊTES sont en premier-écrivain-gagnant (« garder la première arête compatible »). Aucune des deux n’est ce qu’on veut pour une correction de mémoire. Les citations sont l’exception délibérée : unionées par identité avec citation_count = max(existant, entrant, taille de l’union).',
          source: 'recherche §5',
        },
        {
          system: 'graphify — les plafonds',
          fact:
            'MERGE_MAX_GRAPH_BYTES vaut 52 428 800 octets (50 Mio) et la limite de nœuds 100 000 — or le graph.json suivi par git de ce dépôt pèse 54 039 561 octets. LE PILOTE DE FUSION LÈVERAIT DONC AUJOURD’HUI SUR LE GRAPHE COMMITTÉ DE GRAPHIFY LUI-MÊME. La capture vivante ne peut que le faire grossir.',
          source: 'recherche §5 / §15.1',
        },
        {
          system: 'Le corpus',
          fact:
            'Ce que chacun fait vraiment : Hermes = verrou de fichier OS, écriture atomique et détection de dérive (_detect_external_drift() refuse l’écriture et sauvegarde un .bak si le contenu sur disque ne fait pas l’aller-retour du parseur §) — et #56464 a QUAND MÊME perdu des données, dans une seule session, donc pas par concurrence. Letta = verrouillage optimiste plus BlockHistory, le plus solide. Graphiti = pas de verrou, guidage séquentiel, course inter-locataires ouverte. cognee = identifiants déterministes, donc fusion idempotente par identité. ctx = git ordinaire, sans CRDT ni pilote de fusion.',
          source: 'recherche §9',
        },
      ],
      recommendation:
        'Évaluer combien de sessions écriront réellement en parallèle sur le même fait — mais l’arbitrage n’est plus seulement une question de fréquence : la sémantique de fusion actuelle est déjà PLUS FAIBLE qu’un CRDT (aveugle aux suppressions, deux politiques de conflit), donc y aller plus tard voudra dire défaire, pas ajouter. Et le plafond du pilote est déjà dépassé par le graphe committé.',
      nextWork:
        'Recenser les CLI qui écriront simultanément, estimer la fréquence de collision réelle, et trancher séparément le sort du plafond de 50 Mio du pilote de fusion, déjà franchi aujourd’hui.',
    },
    {
      key: 'D7',
      question: 'Quel projet exact recouvre le nom « ctx » cité comme référence ?',
      whyNow:
        'Au moins six projets partagent ce nom avec des mécanismes voisins et des surfaces différentes. Cette révision a cloné et interrogé le candidat retenu, ce qui corrige deux affirmations de la révision 1 et ajoute des chiffres de maturité.',
      options: [
        {
          key: 'activememory-ctx',
          title: 'ActiveMemory/ctx (Go)',
          behavior:
            'Binaire Go unique, local-first, fichiers dans .context/ à côté de .git/, sans embeddings, écritures explicites ; l’agent tire un paquet de contexte par allocation SCORÉE sous budget (ctx agent --budget N) et non par troncature. Correction à la révision 1 : il expose un MCP étendu — ctx mcp en stdio, ctx setup qui génère les configs par outil, ctx steering sync qui pousse les règles dans Claude Code/Cursor/Kiro/Cline, un plugin avec hooks PreToolUse/PostToolUse/UserPromptSubmit et .claude/skills/, ctx loop qui génère un script de boucle autonome, ctx watch qui applique les balises <context-update>. Apache-2.0 et CC-BY-4.0, DCO.',
          consequence:
            'Le plus proche d’un outil git-natif, léger et versionné, et il est bien intégrable en MCP — la révision 1 disait le contraire. La réserve est ailleurs : créé le 20/01/2026, dernier push le 23/07/2026, 1 408 commits, 71 étoiles, 14 forks, 8 issues ouvertes, 11 contributeurs mais TRÈS majoritairement une seule identité — facteur de bus ≈ 1. Étiquettes v0.1.0 à v0.8.0, mais dernière version étiquetée le 24/03/2026 alors que main continue d’avancer sans étiquette : « installer une release » n’est pas « installer main ».',
        },
        {
          key: 'context-llemur',
          ...
          consequence:
            'S’intègre nativement en MCP à plusieurs CLI, mais projet plus jeune et plus petit, à valider en profondeur avant d’en dépendre — et la présence d’un MCP n’est plus un facteur différenciant, puisque le candidat Go en a un aussi.',
        },
        {
          key: 'autre-projet-nomme-ctx',
          title: 'Un autre projet nommé « ctx » (à confirmer)',
          behavior:
            'D’autres projets partagent ce nom sans forcément correspondre au besoin — ctxrs/ctx (Rust, indexe des logs d’agents existants vers SQLite, rappel en lecture seule, 953 étoiles), context-hub/generator (PHP, générateur de doc de code plus MCP).',
          consequence:
            'L’ambiguïté est réelle (6 projets recensés) — il faut faire confirmer lequel était visé avant d’investir. Le même piège existe pour graphify : une recherche web publique sur « graphify » fait remonter Graphify-Labs/graphify, qui n’est PAS ce dépôt.',
        },
      ],
      mechanisms: [
        {
          system: 'ctx (ActiveMemory)',
          fact:
            'La thèse de ctx est celle qui donne le plus de poids à votre instinct sur D3 : « l’humain dans la boucle pour la mémoire » figure parmi les six patrons que 17 systèmes indépendants ont, selon elle, découverts séparément. C’est un argument épistémique, pas un mécanisme — mais il situe une porte de revue comme courante, pas idiosyncrasique.',
          source: 'recherche §7 / §10',
        },
      ],
      recommendation:
        'Confirmer lequel des projets nommés « ctx » était visé, en pesant maintenant la maturité plutôt que la surface : les deux candidats exposent un MCP, et l’écart réel est un facteur de bus proche de 1 et une dernière version étiquetée en mars 2026 alors que main avance encore.',
      nextWork:
        'Faire confirmer le projet visé, puis décider si l’on dépend d’une étiquette de version ou de main, au vu du facteur de bus.',
    },
    {
      key: 'D8',
      ...
      parent: ['D1', 'D4'],
      fromAnswer:
        'Zep/Graphiti ressemble a ce qu’on fait avec graphify. on pourrait donc utiliser a la fois graphify pour la mémoire d’archive et la capture vivante. (D1) — on repose a maxima sur graphify pour cela (évolution bi temporelle a co-design). (D4)',
      question:
        'Jusqu’où graphify porte-t-il À LA FOIS la mémoire d’archive et la capture vivante — et qu’est-ce que cela exige concrètement ?',
      whyNow:
        'Votre réponse ne choisit pas un archétype de la liste : elle demande une COMPOSITION que le benchmark ne contient pas. Elle est défendable — graphify est effectivement plus proche de la cible que la révision 1 ne le disait. Mais cinq mécanismes précis manquent aujourd’hui, et aucun n’est un détail d’effort : ce sont des propriétés absentes, y compris du port de stockage. Les nommer maintenant est moins coûteux que de les découvrir en implémentant.',
      options: [
        {
          key: 'graphify-substrat-unique',
          title: 'Un seul substrat : graphify devient aussi le puits vivant',
          behavior:
            'graphify reçoit ce qui lui manque pour être écrit au fil de l’eau : une écriture par élément (aujourd’hui inexistante), un déclencheur au niveau du tour, une politique d’éviction ou de saillance, une classe d’exception documentée pour une assertion sans localisateur de corpus, et un verrou d’écriture.',
          consequence:
            'Un seul magasin, une seule surface de requête, un seul cycle de vie de revue, une seule chose à sauvegarder. En contrepartie, les cinq mécanismes ci-dessus sont à construire, et l’un d’eux touche l’invariant qui rend graphify digne de confiance : une assertion conversationnelle n’a pas de source_file:page, donc l’invariant de citation devrait gagner une exception explicite — ou les tours devraient devenir des documents de corpus citables.',
        },
        {
          key: 'deux-couches-un-graphe',
          title: 'Deux couches, une promotion sur revue',
          behavior:
            'graphify reste l’archive (batch, adossée aux citations, déclenchée par git) ; une couche de capture distincte tient les assertions conversationnelles et n’est projetée dans graphify qu’à la promotion, après revue.',
          consequence:
            'Préserve l’invariant de citation intact et garde l’aller-retour de graphe complet — environ 620 ms et 373 Mo de RSS — hors du chemin par tour. Mais cela fait deux magasins à opérer et une frontière de promotion à définir : exactement la « frontière claire » que l’option hybride de D1 réclamait déjà, sauf qu’ici elle devient un mécanisme, pas une intention.',
        },
        {
          key: 'capture-hors-graphify',
          title: 'La capture vivante hors de graphify',
          behavior:
            'La capture vit dans un système de mémoire vivante déjà outillé pour ça (routeur mem0, blocs Letta, fichier borné à la Hermes) et graphify n’est jamais un puits vivant.',
          consequence:
            'On obtient gratuitement un chemin d’écriture éprouvé, une éviction et une écriture par fait. Mais on abandonne l’objectif du substrat unique et, pour la mémoire vivante, le cycle de vie citations plus revue — et on ajoute un second modèle de réconciliation à opérer.',
        },
      ],
      mechanisms: [
        {
          ...
          fact:
            'IL N’EXISTE AUCUN UPSERT PAR ÉLÉMENT, nulle part : ni dans le chemin fichier, ni sur le port GraphStore, dont la seule écriture est pushGraph d’un graphe complet. Tout ce qui écrit aujourd’hui réécrit le graphe entier, à environ 620 ms et 373 Mo de RSS à la taille actuelle.',
      ...
      recommendation:
        'Peser lequel des deux invariants coûte le plus cher à perdre : le localisateur de citation sur chaque fait du graphe, ou le substrat unique. C’est le seul arbitrage de cette carte qui ne se règle pas par de l’effort d’implémentation — les quatre autres manques (upsert, déclencheur, éviction, verrou) sont du travail nommable, celui-là est un choix de propriété.',
      nextWork:
        'Trancher en premier l’inconnue non vérifiée-critique ci-dessus (le journal de décisions est-il rejoué à la reconstruction ?), puis décider du sort d’une assertion sans localisateur — les deux conditionnent D9, D11 et D12.',
    },
    {
      key: 'D9',
      addedInRevision: 'agent-memory-2026-07-25',
      parent: ['D2', 'D4'],
      fromAnswer:
        'J’aimerais une approche hybride de graphe entre ontology et bi-temporel.',
      question:
        'Un même graphe à la fois typé par une ontologie ET bi-temporel : que faut-il vraiment ajouter, et où est la dépense réelle ?',
      whyNow:
        'Cette composition n’était pas une option de D2 : les options étaient des archétypes, et vous demandez d’en combiner deux. Un seul système du corpus livre les deux sur le même objet, et la partie chère n’est pas celle qu’on croit — ce n’est pas le champ temporel, c’est ce qui le remplit.',
      options: [
        {
          key: 'tx-additif-sans-detecteur',
          title: 'Ajouter le temps de transaction, sans détecteur de contradiction',
          behavior:
            'On ajoute une paire tx/tx_end à côté du t/t_end existant, en miroir de observed_at et ttl déjà présents dans la provenance d’extraction, plus deux colonnes indexées côté postgres.',
          consequence:
            'C’est ADDITIF ET PEU CHER côté écriture, précisément parce que graphify est déjà uni-temporel (t, t_end, t_iso, t_src, recall --as-of, spécification marquée IMPLÉMENTATION PARTIELLE). Mais sans détecteur, rien ne pose jamais invalid_at : les paires restent des MÉTADONNÉES D’AUDIT et non un mécanisme de gestion de mémoire — et « la bi-temporalité comme gestion de mémoire longue » ne se réalise pas.',
        },
        {
          key: 'detecteur-assertions',
          title: 'Construire la réconciliation d’assertion',
          behavior:
            'On ajoute un détecteur de contradiction qui pose invalid_at — par exemple une opération de patch supersede_relation — en réutilisant le vocabulaire existant (superseded, previous_status).',
          consequence:
            'C’est le seul chemin où la bi-temporalité gère réellement la mémoire. Mais c’est un ALGORITHME DIFFÉRENT de celui que graphify possède : graphify réconcilie des IDENTITÉS, pas des ASSERTIONS. Et c’est exactement la partie chère de Graphiti, dont le coût mesuré par lecture de code est d’environ 2 + 3E + N appels LLM par épisode, linéaire en nombre d’arêtes.',
        },
        {
          key: 'graphiti-couche-vivante',
          title: 'Adopter le seul système qui livre les deux, pour la couche vivante',
          behavior:
            'Graphiti tient la couche vivante : entity_types, edge_types et edge_type_map injectés dans le prompt d’extraction et validés par Pydantic, sur la MÊME EntityEdge qui porte created_at, valid_at, invalid_at et expired_at.',
          consequence:
            'Implémentation fusionnée et éprouvée, sans rien à inventer. Mais il faut un serveur de graphe, un coût LLM linéaire en arêtes, un budget de types plafonné à 10 entités et 10 arêtes (≤ 10 champs, noms d’attributs réservés), et il reste la course inter-locataires ouverte #1331 sur des add_episode concurrents à group_id différents.',
        },
      ],
      mechanisms: [
        {
      ...
      recommendation:
        'Décider si la réconciliation d’ASSERTION entre dans le périmètre AVANT de payer un second axe temporel : sans détecteur qui pose invalid_at, les paires bi-temporelles sont de l’audit, pas de la gestion de mémoire — et c’est cette question, pas le coût des champs, qui décide si la composition demandée délivre ce qu’on en attend.',
      nextWork:
        'Lever d’abord l’incohérence d’intervalle existante, puis prototyper un détecteur de contradiction sur les 3 à 5 scénarios réels de D4 pour mesurer son coût avant de s’engager sur le second axe.',
    },
    {
      key: 'D10',
      addedInRevision: 'agent-memory-2026-07-25',
      parent: ['D2'],
      fromAnswer:
        'la bi-temporalité pourrait être une forme de gestion de mémoire longue au dela de la journée ou simplement du dépassement de contexte (avec des prehook de compaction, l’idéal ?)',
      question:
        'Qu’est-ce qui DÉCLENCHE une écriture en mémoire longue, sachant qu’« écrire à la compaction » n’existe pas comme primitive ?',
      whyNow:
        'Votre question portait un point d’interrogation — « avec des prehook de compaction, l’idéal ? » — et il faut y répondre par un fait, pas par une intuition : le hook de pré-compaction PEUT bloquer mais ne peut RIEN injecter. La frontière de compaction est un point de VETO, pas un point d’écriture. Le choix du déclencheur reste donc entier, et les candidats réels ne mesurent pas le même signal.',
      options: [
        {
          key: 'veto-precompact',
          title: 'Bloquer dans PreCompact, consolider, puis laisser compacter',
          behavior:
            'Le hook PreCompact reçoit session_id, transcript_path, cwd, hook_event_name, trigger et custom_instructions, et peut bloquer (code 2 ou decision: block). On s’en sert pour retarder la compaction le temps d’écrire.',
          consequence:
            'C’est le seul point qui peut RETARDER la perte, et il se déclenche exactement sous la pression de contexte. Mais il ne peut rien réinjecter, il place le travail SYNCHRONEMENT sur le chemin critique de l’humain (au moins environ 620 ms d’aller-retour JSON avant le premier appel LLM), et il consolide ce qui a SURVÉCU au seuil, pas ce qui importait.',
        },
        {
          key: 'compte-de-tours',
          title: 'Compte de tours, ou silence débouncé',
          behavior:
            'On écrit tous les N tours (compteur persisté modulo N de Letta) ou après une pause de conversation (ReflectionExecutor de LangMem, une tâche de fond débouncée), ou sur inactivité horloge (temps de sommeil de Letta).',
          consequence:
            'Le plus prévisible du corpus, et hors du chemin critique — aucune dépendance à un hook de fournisseur. Mais ce signal n’est pas aligné sur la pression de contexte : un tour très long peut perdre de la matière avant que le compteur ne tombe.',
        },
        {
          key: 'commit-git',
          title: 'Le commit git — le déclencheur actuel de graphify',
          behavior:
            'post-commit, post-checkout, post-merge et post-rewrite écrivent needs_update, exportent GRAPHIFY_CHANGED et lancent une reconstruction détachée. C’est déjà câblé et éprouvé.',
          consequence:
            'C’est le SEUL déclencheur du corpus qui coïncide avec une unité d’intention DÉCLARÉE PAR UN HUMAIN — un commit dit « voilà un tout cohérent ». Mais une session peut brûler tout son contexte sans un seul commit : ce déclencheur peut ne jamais se produire quand il faudrait.',
        },
        {
          key: 'frontiere-de-phase',
          title: 'La frontière de phase (compaction intentionnelle fréquente)',
          behavior:
            'On compacte PROACTIVEMENT à 40–60 % d’utilisation, aux frontières de phase (après recherche, après plan, après implémentation), en écrivant des artefacts durables du type research.md, plan.md, progress.md.',
          consequence:
            'C’est de l’art antérieur nommé, avec un argument précis : les frontières de phase sont « les signaux de compaction les plus forts parce qu’elles représentent des points naturels de consolidation de l’information ». Mais cela demande à l’agent de DÉCLARER ses phases : c’est une discipline de harnais, pas un mécanisme que le substrat peut imposer.',
        },
      ],
      mechanisms: [
        {
      ...
      recommendation:
        'Trancher si le déclencheur doit suivre la PRESSION DE CONTEXTE — et alors payer le chemin critique, en sachant qu’on consolidera ce qui a survécu au seuil — ou une UNITÉ D’INTENTION DÉCLARÉE, et alors accepter qu’il puisse ne jamais se déclencher. Le fait qui rend l’arbitrage nécessaire est simple : « écrire la mémoire à la compaction » n’existe pas comme primitive, seul le veto existe, et le retour d’information ne peut se faire qu’au démarrage de la session suivante.',
      nextWork:
        'Mesurer, sur des sessions réelles de cet espace de travail, à quelle fréquence un commit git tombe par rapport à une compaction : c’est ce rapport qui dit si le déclencheur d’intention déclarée suffit ou s’il faut lui adjoindre un compteur de tours.',
    },
    {
      key: 'D11',
      addedInRevision: 'agent-memory-2026-07-25',
      ...
      question:
        'Quel est le statut d’une mémoire capturée mais pas encore revue — et que se passe-t-il quand la revue prend du retard sur la capture ?',
      whyNow:
        'Vous avez nommé une SURFACE de revue (des sessions Focus via h2a), ce qui est cohérent avec la porte choisie en D3. Mais la surface ne dit pas la sémantique, et c’est là qu’un mécanisme précis se retourne : la file de candidats est estampillée par empreinte de graphe, si bien qu’une reconstruction survenue entre la capture et la revue INVALIDE les candidats en attente. Un mécanisme conçu pour protéger la correction se met alors à affamer la revue.',
      options: [
        {
          key: 'file-binaire-invisible',
          title: 'En attente = invisible pour l’agent jusqu’à approbation',
          behavior:
            'La forme de toutes les portes réellement livrées : mise en attente puis approbation, binaire. Chez Hermes tout ce qui n’est pas interactif est mis en attente dans pending/memory/<id>.json ; les skills mettent toujours en attente ; les écritures automatiques sont étiquetées [auto].',
          consequence:
            'La seule forme qui ait un précédent livré, et la réponse prudente au vu des 91 % de pollution promue en exécution d’arrière-plan. Mais tout ce qui n’est pas revu est indisponible : la latence de revue devient directement la latence de la mémoire — et sous capture continue, l’estampille d’empreinte peut invalider des candidats avant qu’un humain les ait vus.',
        },
        {
          key: 'lecture-ponderee',
          title: 'L’agent lit les faits en attente, à un poids de confiance moindre',
          behavior:
            'Le substrat rend les faits non approuvés au rappel, mais dépréciés — la confiance devient continue au lieu d’être binaire.',
          consequence:
            'Le retard de revue cesse de bloquer le rappel. Mais AUCUN système étudié ne livre cela : c’est de la conception de mécanisme neuve, sans art antérieur. Et c’est exactement le chemin de lecture qu’exploite la voie de pollution mesurée par MissClaw.',
        },
        {
          key: 'reduire-la-surface',
          title: 'Garder la porte binaire, mais réduire ce qui atteint l’humain',
          behavior:
            'Des validateurs automatiques s’accordent → promotion automatique ; ils divergent → signalement pour revue humaine. C’est la réponse de SCICERO à l’arriéré.',
          consequence:
            'C’est la réponse de la littérature elle-même (jusqu’à 80 % de précision et 82 % de F1 gagnés « avec un effort manuel minimal ») et elle garde la file à taille humaine. Mais elle exige au moins deux validateurs indépendants, et le résultat de GovMem sur traces réelles était qu’AUCUN chemin de promotion automatique n’était sûr : le bras « promotion automatique » est donc la moitié risquée.',
        },
      ],
      mechanisms: [
        {
      ...
      recommendation:
        'Peser ce qui est le plus coûteux ici : qu’une mémoire vraie reste inaccessible parce que la revue est en retard, ou qu’une mémoire fausse soit lisible parce qu’elle est seulement dépréciée. Et intégrer que la réponse de la littérature à l’arriéré n’est pas de relire plus vite mais de réduire la surface à relire — ce qui déplace la question vers « combien de validateurs automatiques indépendants a-t-on ? ».',
      nextWork:
        'Décider si les candidats en attente doivent survivre à une reconstruction (les ré-ancrer) ou être invalidés, avant de dimensionner une session de revue Focus : c’est ce choix qui détermine si une session de revue hebdomadaire est viable ou structurellement en retard.',
    },
    {
      key: 'D12',
      addedInRevision: 'agent-memory-2026-07-25',
      ...
      options: [
        ...
        },
        {
          ...
          behavior:
            'Le journal append-only devient la source de vérité et le graphe une projection déterministe, rejouable, forkable à n’importe quel événement. Art antérieur nommé : « The Log is the Agent », journal append-only comme source de vérité, graphe comme projection déterministe, fork bon marché.',
          consequence:
            'C’est la cible dont graphify est DÉJÀ LE PLUS PROCHE : trois journaux append-only existent (.graphify/memory/*.md, les patches appliqués et rejetés en JSONL, et le journal de décisions autoritaire), graph.json est déjà un artefact dérivé avec topology_signature, needs_update, built_from_commit et un tri déterministe, et une primitive d’idempotence existe déjà (la clé d’identité de citation). Quatre pièces manquent, toutes nommables : un FOLD qui rejoue le journal de décisions au build, des événements de pierre tombale ou de supersession pour que les suppressions survivent à une union, un mécanisme d’instantané pour borner le rejeu, et une règle pour les opérations non commutatives.',
        },
        {
          key: 'crdt-sur-le-document',
          title: 'Un CRDT JSON greffé sur le document de graphe',
          behavior:
            'On adopte Automerge ou Yjs sur le document, ou un CRDT nativement graphe comme GUN.',
          consequence:
            'Convergence sans coordination. Mais Automerge et Yjs sont des CRDT JSON SANS primitive de graphe, donc SANS intégrité référentielle automatique des arêtes ; la sémantique de suppression est un choix formel (Isolate-Delete, où les arêtes pendantes subsistent isolées, contre Detach-Delete, en cascade, qui exige une livraison causale), et un retrait d’arête concurrent d’un retrait de nœud peut RESSUSCITER les arêtes sortantes d’un nœud supprimé sous une règle naïve. Surtout, c’est la TAILLE DE L’HISTORIQUE D’OPÉRATIONS, pas celle des données, qui tue.',
        },
      ],
      mechanisms: [
        {
    ...
    },
    {
      ...
      mechanisms: [
        ...
        },
        {
          ...
          fact:
            'Vérifié mot pour mot dans la spécification de graphify, dont l’état est « IMPLÉMENTATION PARTIELLE » : « T7 projette dans le graphe agent-stats des preuves de coordination h2a strictement locales ; la mémoire autorisée et la sémantique de persona/connaissance h2a restent NON APPROUVÉES ET HORS PÉRIMÈTRE. » Or D1 et D8 portent précisément sur de la mémoire autorisée. Le statut de cette borne n’est pas une formalité : soit elle est levée explicitement, soit la composition demandée est hors périmètre du document qui la régit.',
      ...
      recommendation:
        'Le critère est un invariant, pas une préférence : peser si l’invariant à trois niveaux du germe — « un substrat, trois niveaux de confiance, ne pas les confondre » — survit aux réponses de ce dossier. La composition à substrat unique de D8 est la seule qui pourrait les confondre ; si l’invariant doit tenir, il contraint D8 et D11 avant qu’elles ne soient tranchées. Et deux appels de ce germe (F2, F3) ne sont pas les vôtres seuls : le pair h2a y est noté comme devant consentir.',
      nextWork:
        'Établir le statut réel du germe (envoyé ou non, étude ouverte ou non), puis trancher explicitement la borne de périmètre de la spécification — mémoire autorisée non approuvée et hors périmètre — avant d’engager D8, car c’est elle qui la franchit.',
    },
  ],
};

// Matrice de comparaison « état de l'art » (PART 2 de la recherche source), CORRIGÉE en révision 2 : les
// cellules que la recherche de mécanismes a démenties sont réécrites, et les corrections sont listées
// explicitement dans `agentMemoryDossier.corrections` — un dossier de décision ne doit pas continuer à
// afficher un fait qu'il sait faux, ni le corriger en silence.
