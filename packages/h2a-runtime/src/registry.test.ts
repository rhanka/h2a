...
const baseInput = {
  ...
  // Enrollment refuses an ambiguous row: every enrolled session declares
  // whether a human was in front of it. A plain `enroll` fixture is a human
  // dev session; the job fixtures below override this to "background".
  sessionClass: "human" as const,
};
...
describe("registry", () => {
  ...
  describe("listLive", () => {
    ...
    it("local liveness follows pid (kill(pid, 0)) and endedAt", () => {
      enroll(
        { id: "with-pid", tool: "codex", kind: "local", cwd: "/x", source: "run", sessionClass: "human", pid: 1234 },
        regPath,
      );
      enroll(
        { id: "dead-pid", tool: "codex", kind: "local", cwd: "/x", source: "run", sessionClass: "human", pid: 9999 },
        regPath,
      );
      enroll(
        { id: "no-pid", tool: "claude", kind: "local", cwd: "/x", source: "hook", sessionClass: "human" },
        regPath,
      );
      enroll(
        { id: "ended", tool: "claude", kind: "local", cwd: "/x", source: "hook", sessionClass: "human" },
        regPath,
      );
    ...
    it("a local entry last seen BEFORE boot is dead even if its PID is now reused", () => {
      ...
      enroll(
        { id: "pre-boot", tool: "claude", kind: "local", cwd: "/x", source: "hook", sessionClass: "human", pid: 1234 },
        regPath,
      );
      const live = listLive({
        path: regPath,
