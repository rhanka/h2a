...
function seed(): { root: string; nestedLeaf: string; engagementLeaf: string; outsideLeaf: string } {
  ...
  const nestedLeaf = track.createItem({
    kind: 'feature',
    title: 'scoped pending 01ARZ3NDEKTSV4RRFFQ69G5FAV work',
    workspace: 'ws',
    parentId: nested,
  ...
  track.createDecision({
    ...
    title: 'choose 01ARZ3NDEKTSV4RRFFQ69G5FAV scoped path',
    workspace: 'ws',
    targets: [nestedLeaf],
    dossier: {
      context: 'recorded context',
      options: [{ id: 'a', title: 'A', summary: 'first 01ARZ3NDEKTSV4RRFFQ69G5FAV' }, { id: 'b', title: 'B', summary: 'second' }],
      qa: [],
...
describe('report --scope', () => {
  ...
  it('derives scoped rows and their journal revision from one event snapshot', () => {
    seed()
    const initialEvents = new EventStore(eventsPath).readAll().length
    const originalReadAll = EventStore.prototype.readAll
    let injected = false
    vi.spyOn(EventStore.prototype, 'readAll').mockImplementation(function (this: EventStore) {
      const events = originalReadAll.call(this)
      if (!injected) {
        injected = true
        // Return the old snapshot while another actor appends before a second read could occur.
        track.createItem({ kind: 'feature', title: 'concurrent work', workspace: 'ws' })
      }
      return events
    })

    const rendered = JSON.parse(reportText(new TrackReader(eventsPath), options, 'json', NOW, false, 'TRACK')) as {
      view: { header: { journalRevision: { events: number; head: string | null } } }
    }

    expect(rendered.view.header.journalRevision.events).toBe(initialEvents)
    expect(rendered.view.header.journalRevision.head).not.toBeNull()
    expect(new TrackReader(eventsPath).cursor().count).toBe(initialEvents + 1)
    vi.restoreAllMocks()
  })
