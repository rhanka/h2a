...
describe('mapWorkEvent — valid kinds → normalized {method, settles, args}', () => {
  const cases: ReadonlyArray<[string, WorkEvent, { method: string; settles: string; args: unknown[] }]> = [
    ...
    ['item.set-raci ⇒ always (creation-payload RACI fields on an existing item)', ev('item.set-raci', { itemId: 'i', accountable: 'human:alice', responsible: ['agent:codex'] }),
      { method: 'setRaci', settles: 'always', args: ['i', { accountable: 'human:alice', responsible: ['agent:codex'] }] }],
    ...
    // Regression expression (2.1.0) — a reopening always settles (it moves a WP percentage) and carries its motive.
    ['item.reopen ⇒ always (reopen a closed item WITH its motive)', ev('item.reopen', { itemId: 'i', motive: 'regression-observed', reason: 'the delivered capability is broken' }),
      { method: 'reopenItem', settles: 'always', args: ['i', { motive: 'regression-observed', reason: 'the delivered capability is broken' }] }],
    ['decision.create', ev('decision.create', { decisionKind: 'orientation', title: 'D', workspace: 'ws', targets: ['i'], dossier: { context: '', options: [{ id: 'a', title: 'Option A', summary: 'first option' }, { id: 'b', title: 'Option B', summary: 'second option' }], qa: [], recommendation: { optionId: 'a', rationale: 'Option A is recommended' } } }),
      { method: 'createDecision', settles: 'never', args: [{ decisionKind: 'orientation', title: 'D', workspace: 'ws', targets: ['i'], dossier: { context: '', options: [{ id: 'a', title: 'Option A', summary: 'first option' }, { id: 'b', title: 'Option B', summary: 'second option' }], qa: [], recommendation: { optionId: 'a', rationale: 'Option A is recommended' } } }] }],
    ['decision.dossier', ev('decision.dossier', { decisionId: 'd', dossier: { context: 'x', options: [{ id: 'a', title: 'Option A', summary: 'first option' }, { id: 'b', title: 'Option B', summary: 'second option' }], qa: [], recommendation: { optionId: 'a', rationale: 'Option A is recommended' } } }),
      { method: 'reviseDossier', settles: 'never', args: ['d', { context: 'x', options: [{ id: 'a', title: 'Option A', summary: 'first option' }, { id: 'b', title: 'Option B', summary: 'second option' }], qa: [], recommendation: { optionId: 'a', rationale: 'Option A is recommended' } }] }],
    ['decision.select ⇒ always', ev('decision.select', { decisionId: 'd', optionId: 'a' }),
      { method: 'selectDecisionOption', settles: 'always', args: ['d', 'a', 'go'] }],
    ['decision.outcome ⇒ authenticated defer only', ev('decision.outcome', { decisionId: 'd', to: 'deferred' }),
      { method: 'setOutcome', settles: 'always', args: ['d', 'deferred'] }],
