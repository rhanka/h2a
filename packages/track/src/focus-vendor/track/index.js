...
export class TrackContractMismatchError extends Error {
    actual;
    expectedMajor;
    constructor(actual, expectedMajor) {
        super(`@sentropic/track/read contract ${actual} is incompatible with focus/track ` +
...
const outcomeStatement = (outcome) => {
    switch (outcome) {
        case "go":
 ...
 */
const collectComprehension = (dossier) => {
    const out = [];
...
const toFocusAmendmentStep = (step) => ({
    at: step.at,
    ...
    if (dossier.options.length > 0) {
        sections.push({
            ...
            options: dossier.options.map((opt) => ({
                ...
                annotation: opt.id === dossier.selectedOptionId ? "accept" : "none",
 ...
 */
export const toDecisionDossierDocument = (view, amendmentTrace, meta) => {
 ...
 */
export const readDecisionDossier = (eventsPath, query, readAt) => {
    const reader = new TrackReader(eventsPath);
    if (majorOf(reader.contractVersion) !== EXPECTED_TRACK_READ_MAJOR) {
        throw new TrackContractMismatchError(reader.contractVersion, EXPECTED_TRACK_READ_MAJOR);
    }
