...
const renderQuestion = (q) => {
    ...
    if (q.recommendedAnswer !== undefined) {
        parts.push(`<p class="focus-preco">préco: ${escapeHtml(q.recommendedAnswer)}</p>`);
    }
...
const renderOptionSet = (s) => {
    const head = s.title !== undefined
        ? `<h3>Options — ${escapeHtml(s.title)}</h3>`
        : "<h3>Options</h3>";
    const items = s.options.map((o) => {
        const comment = o.annotationComment !== undefined
...
const renderAmendmentTrace = (steps) => {
    if (steps.length === 0) {
        return `<section class="focus-amendment-trace"><h3>Amendment trace</h3><p><em>(none)</em></p></section>`;
    }
    const items = steps.map((st) => `<li><code>${escapeHtml(st.at)}</code> <strong>${escapeHtml(st.author)}</strong> — ${escapeHtml(st.summary)}</li>`);
    return `<section class="focus-amendment-trace"><h3>Amendment trace</h3><ol>${items.join("")}</ol></section>`;
};
const renderDiagram = (d) => `<figure class="focus-diagram" data-syntax="${escapeHtml(d.syntax)}"><figcaption>${escapeHtml(d.alt)}</figcaption><pre><code class="language-${escapeHtml(d.syntax)}">${escapeHtml(d.source)}</code></pre></figure>`;
const renderNode = (node, hooks) => {
    switch (node.kind) {
        case "prose":
...
const renderAffordances = (affordances) => {
    if (affordances.length === 0)
        return "";
    const items = affordances.map((a) => `<li class="focus-affordance" data-affordance="${escapeHtml(a.name)}" data-target="${escapeHtml(a.targetRef)}" aria-disabled="true"><del>${escapeHtml(a.label)}</del> <span class="focus-affordance-meta">${escapeHtml(a.name)} → ${escapeHtml(a.targetRef)} (disabled)</span></li>`);
...
export const renderHtml = (doc, hooks) => {
    const header = `<header class="focus-header"><h1>${escapeHtml(doc.title)}</h1>` +
    ...
    const html = `<article class="focus-document" data-ref="${escapeHtml(doc.ref)}">` +
        header +
        sections +
        amendment +
        affordances +
