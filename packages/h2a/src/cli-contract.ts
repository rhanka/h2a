...
export const H2A_CLI_VERB_CONTRACTS: readonly H2ACliVerbContract[] = [
  ...
  },
  {
    ...
    description:
      "Prove control of the LIVE agent key over a gateway-issued enrollment challenge, and print the `{ nonce, signature, publicKeyPem, instance }` proof, whose signature covers every field except itself. Proof of AUTHORSHIP only, never of authorization: the 39-auth principal authorizes and sentropic mints/stores the binding. Resolves the live identity at run time (`--instance` is refused — a recorded id names a re-anchored key) and makes NO network call. Stores no binding, challenge or proof — but identity resolution may mint and persist the local keypair, registry row, identity binding and alias exactly as `connect` does, so on a machine that never connected this verb creates a durable local identity. The challenge must not carry a `principalSub`. Exit 1 on a bad/expired challenge, 2 when the local key state is unusable. Part B of the 2026-07-24 session-exposure feed contract."
  },
  {
    verb: "nhi report",
    outputShape: "resource",
