...
export const H2A_CLI_VERB_CONTRACTS: readonly H2ACliVerbContract[] = [
  ...
  },
  {
    ...
    exitCodes: [0, 2, 3],
    requiredFlags: [],
    optionalFlags: ["root", "scan", "prune", "repair", "dry-run"],
    description:
      "Diagnose the h2a store: root provenance, split-brain repo-local bus, inbox hygiene (case-dup / host-less / phantom dirs), and (--scan <dir>) stray repo-local buses. Hard checks (rootExists, schemaSentinel, liveSessions, cliBinary) drive exit 2; soft checks surface as `warnings[]` without flipping ok. `--repair` is the single opt-in Claude/Codex installation convergence action: it detects and repairs plugin marketplace/cache/version drift plus singleton H2A/Track MCP configuration, removes stale H2A cache/config residue, and reports every native-host command failure as unrepaired. `--repair --dry-run` runs that full diagnosis and lists planned host actions without changing configuration, caches, markers, or native-host state. A live H2A session that predates a recorded repair remains not clean until restart. Doctor guarantees the coherence of repairs it performed; it does not detect external installation changes, for which the operator must restart sessions. If a native host CLI fails after it has already changed the installation, doctor reports the failure as host-command-failed and does not undo what that CLI already did. Doctor's own configuration writes are atomic. It has no snapshot of third-party state and does not simulate one: a partial restore would promise a recovery it cannot deliver. After a reported native failure, verify the host installation before relying on it. Without `--repair`, doctor remains the non-mutating bus-health probe and does not inspect personal host configuration. --prune (opt-in) DELETES the clearly-dead artifacts the report identifies: host-less inbox dirs, phantom 3-segment dirs, orphan-uuid inbox dirs, and (with --scan) stray buses — report.pruned lists what was removed. Each --scan bus is liveness-checked via listPresence(): a bus with ANY fresh presence heartbeat is `live` and is NEVER pruned, --prune or not; only `orphan` (no live presence) buses are deletion candidates. caseDuplicates and registered-offline dirs are NOT pruned (too risky). Default is dry-run — the report always lists what --prune would remove before anything is deleted. DEC-054."
  },
  {
    verb: "status",
    outputShape: "action",
