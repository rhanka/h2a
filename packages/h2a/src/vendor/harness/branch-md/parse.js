...
function backtickGlobs(line) {
    const out = [];
    for (const m of line.matchAll(BACKTICK))
        out.push(m[1]);
    return out;
}
export function parseBranchMd(text) {
    ...
    let inPlan = false;
    let currentLot = null;
    const pushBucket = (globs) => {
        if (bucket === 'allowed')
            result.allowedPaths.push(...globs);
        else if (bucket === 'forbidden')
            result.forbiddenPaths.push(...globs);
        else if (bucket === 'conditional')
            result.conditionalPaths.push(...globs);
    };
    for (const line of lines) {
        for (const m of line.matchAll(EXCEPTION))
            result.exceptions.push(m[0]);
        if (/^##\s+/.test(line)) {
            bucket = null;
