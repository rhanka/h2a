...
export const VIOLATION_SEVERITIES = ['advisory', 'blocking'];
const ACCEPTANCE_KINDS = ['unit', 'integration', 'e2e', 'manual'];
function isRecord(v) {
...
function validateViolation(v, path, errors) {
    if (!isRecord(v)) {
        errors.push(`${path}: expected object`);
        return;
    }
    if (!isStr(v.code))
        errors.push(`${path}.code: expected string`);
    if (!isStr(v.message))
        errors.push(`${path}.message: expected string`);
    if (!VIOLATION_SEVERITIES.includes(v.severity)) {
        errors.push(`${path}.severity: expected one of ${VIOLATION_SEVERITIES.join('|')}`);
    }
    if (v.path !== undefined && !isStr(v.path))
        errors.push(`${path}.path: expected string`);
}
function validateTarget(t, path, errors) {
    ...
    if (t.acceptance !== undefined) {
        ...
        if (!isRecord(a)) {
            ...
            if (!isStr(a.evidenceId))
                errors.push(`${path}.acceptance.evidenceId: expected string`);
            if (!ACCEPTANCE_KINDS.includes(a.kind)) {
...
function validateCheck(c, path, mode, errors) {
    if (!isRecord(c)) {
        errors.push(`${path}: expected object`);
        return;
    }
 ...
 */
export function validateVerificationRunV0(run, opts = {}) {
    ...
    if (run.schemaVersion !== 1)
        errors.push('$.schemaVersion: expected 1 (unknown version fails closed)');
    for (const key of ['runId', 'commit', 'branch', 'env', 'runner', 'command', 'artifactLocator']) {
        if (!isStr(run[key]))
            errors.push(`$.${key}: expected string`);
    }
    if (!VERIFICATION_CATEGORIES.includes(run.category)) {
    ...
    if (!isStr(run.startedAt))
        errors.push('$.startedAt: expected ISO-8601 string');
    if (!isStr(run.finishedAt))
        errors.push('$.finishedAt: expected ISO-8601 string');
 ...
 */
export function deriveVerdict(violations) {
    if (violations.some((v) => v.severity === 'blocking'))
        return 'violation';
    if (violations.some((v) => v.severity === 'advisory'))
        return 'conditional';
    return 'clean';
}
