...
const CATEGORIES = ['none', 'static', 'unit', 'integration', 'e2e', 'ci', 'uat'];
export const MECHANICAL_VERBS = ['verify', 'init', 'audit'];
function isMechanicalVerb(v) {
    return v !== undefined && MECHANICAL_VERBS.includes(v);
}
...
function runContext(flags, category, command) {
    return {
        runId: 'cli',
...
function staticChecks(flags, profile, out) {
    const checks = [];
    if (flags['staged-files'] !== undefined || flags['branch-md'] !== undefined) {
        ...
        if (outcome.result)
            checks.push({ code: 'C2', category: 'static', result: outcome.result });
    }
...
function emitRun(checks, flags, category, command, out) {
 ...
 */
export function handleMechanicalVerb(positionals, flags, out) {
    const verb = positionals[0];
    if (!isMechanicalVerb(verb))
        return null;
