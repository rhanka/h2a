...
import { parseFlags, str } from './args.js';
...
function handleCheck(positionals, flags, out) {
    const sub = positionals[1];
    if (sub !== 'scope' && sub !== 'branch') {
        out(USAGE);
        return 2;
    }
    const profile = str(flags.profile) === 'stub' ? stubProfile : sentropicProfile;
 ...
 */
export function runHarnessCli(argv, out) {
    ...
    if (mechanicalCode !== null)
        return mechanicalCode;
