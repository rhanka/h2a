...
export const METHOD_VERBS = ['brainstorm', 'debug', 'review', 'plan', 'test', 'branch', 'skills'];
export function isMethodVerb(v) {
    return v !== undefined && METHOD_VERBS.includes(v);
}
/** Pure work-event context — the caller injects commit/branch/env; the driver passes placeholders. */
export function workContext(flags) {
    return {
        runId: 'cli',
...
function header(verb, status, skill) {
    return `harness ${verb} — ${status}${skill ? ` (skill: ${skill})` : ''}`;
}
function build(verb, positionals, flags) {
 ...
 */
export function handleMethodVerb(positionals, flags, out) {
    const verb = positionals[0];
    if (!isMethodVerb(verb))
        return null;
